...
const miejsce = require('./miejsce.js');
...
function utworz({
  ...
  function handleLogout(req, res) {
    ...
    res.writeHead(200, {
      ...
      // Z tymi samymi atrybutami co przy logowaniu — przeglądarka ma nadpisać TO ciastko.
      'Set-Cookie': `cosmos_auth=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0${AUTH.cookieSecure ? '; Secure' : ''}`,
    });
    res.end('{"ok":true}');
  }
  ...
  async function handleZaproszenie(req, res) {
    ...
    if (req.method === 'GET') {
      /* Token w NAGŁÓWKU, nie w adresie: adres trafia do logów Cloudflare,
         a dokumentacja obiecuje, że token z linku nigdzie nie zostaje.
         `?token=` zostaje dla starej wersji aplikacji z pamięci telefonu. */
      const token = String(req.headers['x-cosmos-zaproszenie']
  ...
  async function handleKonto(req, res, p) {
    ...
    if (p === '/api/konto' && req.method === 'GET') {
      return sendJson(res, 200, {
        ...
        // Ile zajmują moje dane i jaki mam limit (0 = bez limitu) — lib/miejsce.js.
        miejsce: await miejsce.stanMiejsca(u),
      });
    }
    /* Puste ciało to nie błąd: „Wyloguj wszędzie" wysyła POST bez treści,
       a `JSON.parse('')` dawał 400 — przycisk bezpieczeństwa nie działał
       nigdy. Nieprawidłowy JSON dalej jest błędem. */
    let d = {};
    try {
      const surowe = (await readBodyBuffer(req, 64 * 1024)).toString('utf8');
      if (surowe.trim()) d = JSON.parse(surowe);
    } catch { return sendJson(res, 400, { error: 'Nieprawidłowy JSON.' }); }
  ...
  async function handleKonta(req, res, p) {
    ...
    if (p === '/api/konta' && req.method === 'GET') {
      return sendJson(res, 200, {
        logowanie: authEnabled(),
        /* Zajęte miejsce każdej osoby: właściciel nie widzi cudzych treści,
           ale musi widzieć, kto zapełnia dysk VPS-a. */
        uzytkownicy: await Promise.all(konta.wszyscy().map(async (x) => ({
          ...x, sesji: konta.ileSesji(x.id), miejsce: await miejsce.stanMiejsca(x),
        }))),
        dysk: miejsce.wolneNaDysku(DATA_DIR),
