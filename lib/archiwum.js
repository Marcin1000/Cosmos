...
function utworz(katalogDanych) {
   ...
   *
   *  Marcin fotografuje w RAW+JPEG, więc w archiwum każdy kadr leży dwa razy.
   *  Ma to znaczenie przy miniaturach: dla CR3 Microsoft Graph musi wygenerować
   *  podgląd u siebie i zmierzone trwa to 6,8 s, dla JPG oddaje gotowy plik.
   *  Znając rodzeństwo, pytamy o tańszy.
   *
   *  Grupujemy po pełnej ścieżce bez rozszerzenia, więc `/Mazury/A.CR3`
   *  i `/Wesele/A.CR3` zostają osobno.
   *
   *  Indeks budujemy przy pierwszym pytaniu i przebudowujemy, gdy zmieni się
   *  liczba wpisów — ścieżka istniejącego wpisu się nie zmienia, więc nic
   *  innego nie może go unieważnić. Trzymamy identyfikatory, nie obiekty:
  ...
  function rodzenstwo(id) {
    ...
    if (!w) return [];
    if (!w.sciezka) return [w];
    if (rodzenstwoDla !== wpisy.length) {
      rodzenstwoIndeks = new Map();
      for (const x of wpisy) {
        if (!x.sciezka) continue;
        const k = bezRozszerzenia(x.sciezka);
        if (!rodzenstwoIndeks.has(k)) rodzenstwoIndeks.set(k, []);
        rodzenstwoIndeks.get(k).push(x.id);
      }
      rodzenstwoDla = wpisy.length;
    }
    const ids = rodzenstwoIndeks.get(bezRozszerzenia(w.sciezka));
    if (!ids) return [w];
