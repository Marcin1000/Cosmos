...
function utworz(katalogDanych) {
  ...
  let wpisy = [];
  let poId = new Map();
  /* Licznik zmian daty lub ścieżki na ISTNIEJĄCYCH wpisach. Unieważnia indeks
     rodzeństwa, który liczy się właśnie z tych dwóch pól — a dociąganie EXIF-u
     podmienia datę wgrania na datę zrobienia zdjęcia, nie zmieniając przy tym
     liczby wpisów. Bez tego licznika po dociągnięciu danych parowalibyśmy
     kadry po nieaktualnych datach. */
  let wersjaKadrow = 0;
  ...
  function dodaj(nowe) {
    ...
    for (const w of nowe || []) {
      ...
      if (poId.has(wpis.id)) {
        ...
        if (stary.kiedy !== wpis.kiedy || stary.sciezka !== wpis.sciezka) wersjaKadrow++;
        wpisy[i] = wpis;
        odswiezonych++;
      } else {
        wpisy.push(wpis);
   ...
   *
   *  Indeks budujemy przy pierwszym pytaniu i przebudowujemy, gdy zmieni się
   *  liczba wpisów. Uwaga: `kiedy` POTRAFI się zmienić bez zmiany liczby
   *  wpisów — dociąganie EXIF-u podmienia datę wgrania na datę zrobienia
   *  zdjęcia. Dlatego indeks znaczymy też licznikiem takich podmian
   *  (`wersjaKadrow`), inaczej po dociągnięciu danych trzymalibyśmy klucze
   *  policzone ze starych dat. Trzymamy identyfikatory, nie obiekty:
   *  `dodaj()` PODMIENIA obiekt wpisu przy odświeżeniu, więc referencje
   *  zwietrzałyby po pierwszym dociągnięciu EXIF-u. */
  ...
  let rodzenstwoDla = -1;
  let rodzenstwoWersja = -1;
  function rodzenstwo(id) {
    const w = poId.get(String(id));
    if (!w) return [];
    if (!w.sciezka && !w.nazwa) return [w];
    if (rodzenstwoDla !== wpisy.length || rodzenstwoWersja !== wersjaKadrow) {
      ...
      rodzenstwoDla = wpisy.length;
      rodzenstwoWersja = wersjaKadrow;
    }
