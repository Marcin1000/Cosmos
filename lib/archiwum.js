...
/** Ile wpisów liczymy jednym ciągiem, zanim oddamy pętlę zdarzeń (dodajPorcjami). */
const PORCJA_DODAWANIA = 200;
...
function utworz(katalogDanych) {
  ...
  /* Pozycja wpisu w `wpisy` po id. `indexOf` przy każdej aktualizacji był
     niewinny dla 500 plików (132 ms, tempo-archiwum), ale ponowne
     indeksowanie 5000 plików na 57 tysiącach to już setki milionów porównań.
     Budowana leniwie, gubiona przy przestawieniu tablicy (przebuduj). */
  let pozycje = null;

  // Brak indeksu = pierwszy raz; uszkodzony — kopia obok, odbudowa przyciskiem „Indeksuj teraz".
  const surowe = czytajJson(PLIK, []);
  ...
  function pozycjaWpisu(id, obiekt) {
    if (!pozycje) pozycje = new Map(wpisy.map((w, j) => [w.id, j]));
    const i = pozycje.get(id);
    if (i !== undefined && wpisy[i] === obiekt) return i;
    // Tablicę przestawiono bez nas — raz liniowo i mapa od nowa.
    pozycje = null;
    return wpisy.indexOf(obiekt);
  }
   ...
   */
  let zapisObietnica = null;
  function zapisz() {
    if (zapisTrwa) { zapisZaległy = true; return zapisObietnica || Promise.resolve(true); }
    zapisObietnica = zapiszTeraz();
    return zapisObietnica;
  }

  /** Zapis, który obejmuje WSZYSTKO do tej chwili, z wynikiem: true = na dysku.
   *  `zapisz()` w trakcie trwającego zapisu oddaje obietnicę TAMTEGO zapisu —
   *  bez wpisów dodanych w międzyczasie. Kolejka OneDrive, zapisywana zaraz po
   *  nim, odhaczała wtedy strony, których na dysku jeszcze nie było (zespół IT,
   *  runda 4). */
  async function zapiszPoTeraz() {
    while (zapisTrwa && zapisObietnica) await zapisObietnica;
    return zapisz();
  }

  /** Przy zamykaniu serwera: zapis tylko wtedy, gdy coś na niego czeka —
   *  odłożony zapis albo zapis w toku (i to, co przyszło w jego trakcie).
   *  Dawniej zamknięcie zapisywało 100-megabajtowy indeks zawsze, także bez
   *  żadnej zmiany. */
  async function zapiszZalegle() {
    if (zapisTrwa && zapisObietnica) await zapisObietnica;
    if (!zapisZaplanowany) return;
    clearTimeout(zapisZaplanowany);
    zapisZaplanowany = null;
    await zapisz();
  }

  async function zapiszTeraz() {
    zapisTrwa = true;
    ...
    try {
      ...
      // `sync` przed podmianą — jak zapiszAtomowo w lib/rdzen.js: zanik zasilania
      // po `rename` bez niego potrafi zostawić plik pusty.
      const uchwyt = await fs.promises.open(tmp, 'w');
      try {
        await uchwyt.writeFile(JSON.stringify(wpisy));
        await uchwyt.sync();
      } finally {
        await uchwyt.close();
      }
      ...
      // Resztka po nieudanym zapisie (pełny dysk) zjadałaby miejsce wszystkim — patrz zapiszAtomowo.
  ...
  /* PONOWNE INDEKSOWANIE NIE MOŻE KASOWAĆ PRACY.
   *
   *  Dotąd `dodaj()` podmieniało wpis w całości: `wpisy[i] = nowy`. Dla
   *  nowego pliku poprawne, dla znanego — katastrofalne. Marcin przemielił
   *  55 tysięcy plików rozpoznawaniem treści (godziny pracy karty graficznej)
   *  i osobnym przebiegiem po EXIF. Wystarczyłoby, żeby kliknął „indeksuj
   *  OneDrive" po dograniu nowej sesji — a to jest normalna, oczekiwana
   *  czynność — i cała ta praca znikałaby bez jednego ostrzeżenia:
   *  `obejrzane`, `obiekty`, `obiektyw`, dane lotu i poprawione daty
   *  wracałyby do stanu sprzed pierwszego uruchomienia. Indeksowanie
   *  wyglądałoby przy tym na udane, bo pliki by się zgadzały co do liczby.
   *
   *  Reguła jest prosta i wynika z tego, KTO co wie:
   *  — listowanie z Grapha zna nazwę, ścieżkę, rozmiar i to, co Microsoft
   *    sam wyczytał z JPEG-a; nie zna niczego, co policzyliśmy później,
   *  — przebiegi po EXIF i po obrazie znają resztę.
   *  Więc nowy wpis wygrywa tam, gdzie faktycznie coś niesie, a pola
   *  z późniejszych przebiegów przeżywają, dopóki nowy ich nie wypełni.
   *
   *  Data jest wyjątkiem wymagającym osobnej reguły, bo Graph ZAWSZE
   *  poda jakąś: dla RAW-a jest to moment wgrania pliku do chmury. Gdyby
   *  „niepusta wygrywa", to jedno przeindeksowanie cofałoby zdjęcia z gór
   *  spod prawdziwego 2022 pod fałszywy 2026 — czyli dokładnie usterkę,
   *  którą przebieg po EXIF naprawił. Dlatego daty porównujemy po ŹRÓDLE:
   *  exif > nazwa pliku > data pliku, i gorsze nigdy nie nadpisuje lepszej.
   */
  const RANGA_DATY = { exif: 3, nazwa: 2, plik: 1 };
  /* Pola, których listowanie z Grapha NIE ZNA — wypełniają je późniejsze
     przebiegi. Puste w nowym wpisie znaczy „nie sprawdzałem", nie „nie ma". */
  const POLA_Z_PRZEBIEGOW = ['obiektyw', 'dron', 'lot', 'miejsce',
    'aparat', 'iso', 'przyslona', 'czasS', 'ogniskowa', 'lat', 'lon'];

  function scal(stary, nowy) {
    const out = { ...nowy };
    for (const pole of POLA_Z_PRZEBIEGOW) {
      const brak = out[pole] === null || out[pole] === undefined || out[pole] === '';
      if (brak && stary[pole] !== null && stary[pole] !== undefined) out[pole] = stary[pole];
    }
    // Flagi „już to robiliśmy" i owoce tych przebiegów — nigdy nie cofamy.
    if (stary.obejrzane) out.obejrzane = true;
    if (stary.exifCzytany) out.exifCzytany = true;
    if (!out.obiekty.length && stary.obiekty.length) out.obiekty = stary.obiekty;

    /* Data: wygrywa LEPSZE źródło, a przy równym — nowsze. Równe źródło
       znaczy „ten sam sposób odczytu, tylko świeższy" (np. powtórzony
       przebieg po EXIF), więc blokowanie go zamieniłoby ochronę danych
       w zamrażarkę: poprawki wniesione w Lightroomie nigdy by nie weszły.

       Wpisy sprzed wprowadzenia `dataZrodlo` nie mają go wcale i wyszłyby
       z rangą zero — czyli ponowne indeksowanie nadpisałoby datą wgrania
       właśnie te 55 tysięcy plików Marcina, którym przebieg po EXIF już
       raz datę naprawił. `exifCzytany` mówi o nich to samo, co powiedziałoby
       `dataZrodlo`, więc czytamy je jako 'exif'. */
    const ranga = (w) => RANGA_DATY[w.dataZrodlo] || (w.exifCzytany && w.kiedy ? 3 : 0);
    const rNowa = ranga(nowy);
    const rStara = ranga(stary);
    if (stary.kiedy && (!out.kiedy || rStara > rNowa)) {
      out.kiedy = stary.kiedy;
      out.dataZrodlo = stary.dataZrodlo;
    }

    /* Pola wyliczane trzeba policzyć PONOWNIE, bo przed chwilą podmieniliśmy
       ich składniki. Bez tego zdjęcie odzyskiwałoby datę i GPS z EXIF-u,
       ale zostawało z porą światła policzoną dla daty wgrania. */
  ...
  function dodaj(nowe) {
    ...
    for (const w of nowe || []) {
      if (!w || !w.id) continue;
      let wpis = znormalizuj(w);
      if (poId.has(wpis.id)) {
        const surowyStary = poId.get(wpis.id);
        const i = pozycjaWpisu(wpis.id, surowyStary);
        /* WPISY Z DYSKU WCZYTUJĄ SIĘ SUROWE, bez normalizacji — i to jest
           celowe, bo normalizacja 57 tysięcy wpisów przy każdym starcie
           znaczyłaby tyleż obliczeń pozycji Słońca. Ale `scal()` zakłada
           pełny kształt: sięga po `stary.obiekty.length`, a wpis zapisany
           przez starszą wersję tego pola nie ma. Zmierzone: pierwsze
           `dodaj` na takim wpisie rzucało „Cannot read properties of
           undefined (reading 'length')" i przerywało CAŁĄ paczkę — nie
           jeden plik, tylko całe indeksowanie.

           Normalizujemy więc leniwie: tylko wpisy, których naprawdę
           dotykamy, i tylko gdy wyglądają na stare. Przy okazji je to
           naprawia, więc problem znika sam wraz z pierwszym przeindeksowaniem. */
        const stary = Array.isArray(surowyStary.obiekty)
          ? surowyStary : znormalizuj(surowyStary);
        wpis = scal(stary, wpis);
        przepniKadr(surowyStary, wpis);
        wpisy[i] = wpis;
        odswiezonych++;
      } else {
        przepniKadr(null, wpis);
        wpisy.push(wpis);
        if (pozycje) pozycje.set(wpis.id, wpisy.length - 1);
        dodanych++;
      }
  ...
  /** To samo co `dodaj`, ale porcjami, z oddaniem pętli zdarzeń między nimi.
   *
   *  Paczka 5000 wpisów liczyła się jednym ciągiem ~2 s (pozycja Słońca,
   *  tematy — na każdy plik) i przez ten czas stały odpowiedzi WSZYSTKICH
   *  osób, strumienie czatu też. Porcja 200 to ~80 ms pracy, potem serwer
   *  obsługuje, co na niego czeka. Wynik ten sam co `dodaj`. */
  async function dodajPorcjami(nowe, { porcja = PORCJA_DODAWANIA } = {}) {
    const lista = nowe || [];
    const suma = { dodanych: 0, odswiezonych: 0, razem: wpisy.length };
    for (let i = 0; i < lista.length; i += porcja) {
      const w = dodaj(lista.slice(i, i + porcja));
      suma.dodanych += w.dodanych;
      suma.odswiezonych += w.odswiezonych;
      suma.razem = w.razem;
      if (i + porcja < lista.length) await new Promise((ok) => setImmediate(ok));
    }
    return suma;
  }
  ...
  function znormalizuj(w) {
    const out = {
      ...
      kiedy: w.kiedy || null,
      /* SKĄD JEST DATA: 'exif' | 'nazwa' | 'plik'. Bez tego pola nie da się
         odróżnić momentu zrobienia zdjęcia od momentu wgrania go do chmury —
         a przy RAW-ach Graph podaje wyłącznie to drugie. Decyduje o tym,
         czy ponowne indeksowanie ma prawo nadpisać datę (patrz `scal`),
         i o tym, czy model wolno jej użyć jako faktu. */
      dataZrodlo: ['exif', 'nazwa', 'plik'].includes(w.dataZrodlo) ? w.dataZrodlo : null,
  ...
  function szukaj(f = {}) {
    ...
    /* WYKLUCZENIE FOLDERU. „Pokaż najnowsze zdjęcia oprócz tych z Mazur"
       nie miało jak zadziałać: filtrów było dwadzieścia i ani jednego
       odejmującego. Model układał wtedy zapytanie bez wykluczenia i pisał
       w odpowiedzi, że folder pominął — co bywało prawdą przez przypadek
       (gdy najnowsze pliki i tak leżały gdzie indziej), a bywało nieprawdą.
       Kilka nazw po przecinku, bo „oprócz Mazur i Krakowa" to jedno pytanie. */
    if (ma(f.bezFolderu)) {
      const igly = String(f.bezFolderu).split(',').map((s) => bezOgonkow(s.trim())).filter(Boolean);
      if (igly.length) {
        out = out.filter((w) => !igly.some((i) => bezOgonkow(w.sciezka).includes(i)));
      }
    }

    /* Teksty dopasowujemy PO SŁOWACH, nie całą frazą.
       EXIF Canona zapisuje „Canon EOS R6m2”, a OneDrive pokazuje „EOS R6
       Mark II” — więc naturalne „aparat=Canon R6” nie trafiało w nic, bo
       jako jeden ciąg nie występuje nigdzie. Rozbicie na słowa („canon”
       i „r6” muszą być OBA) trafia w każdy z tych zapisów i nadal nie myli
       R6 z R5. Pojedyncze słowo działa jak dotąd: „R6” znajdzie „R6m2”. */
    for (const pole of ['aparat', 'obiektyw', 'miejsce', 'nazwa']) {
      if (ma(f[pole])) {
        const slowa = bezOgonkow(f[pole]).split(/\s+/).filter(Boolean);
        out = out.filter((w) => {
          const stog = bezOgonkow(w[pole] || '');
  ...
  return { dodaj, dodajPorcjami, szukaj, zestawienie, podsumowanie, usunZrodlo, zapisz, zapiszPoTeraz, zapiszZalegle,
