...
const { SECRETS, genId, readJson, saveJsonFile, sendJson } = require('./rdzen.js');
...
function czytajJson(plik) {
  try { return JSON.parse(fs.readFileSync(plik, 'utf8')); } catch { return []; }
}
function N() {
  return stan('nauka', (katalog) => ({
    katalog,
    lessons: czytajJson(path.join(katalog, 'lessons.json')),
...
async function handleProcedures(req, res, pathname) {
  ...
  if (pathname === '/api/procedures' && req.method === 'DELETE') {
    ...
    N().routines = N().routines.filter((r) => r.procedureId !== id); saveRoutines();
    saveProcedures();
    return sendJson(res, 200, { ok: true });
  }
  res.writeHead(405); res.end();
