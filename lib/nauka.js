...
let KB_FILES, addEvent, cosine, embedTexts, kbAddFile, kbItems, keywordScore, sameModel, saveKb, tsName;
function polacz(z) {
  ({ KB_FILES, addEvent, cosine, embedTexts, kbAddFile, kbItems, keywordScore, sameModel, saveKb, tsName } = z);
}
...
function uzupelnijWzorceWTle(qmodel) {
  ...
  const brakujace = lessons.filter((l) => !sameModel(l, qmodel));
  ...
  setTimeout(async () => {
    try {
      ...
      if (embs) {
        brakujace.forEach((l, i) => { l.embedding = embs.vectors[i]; l.embModel = embs.model; });
        saveLessons();
      }
