...
/** `sufit` – najwięcej tokenów, ile wolno tej osobie na tym silniku (lib/silniki.js,
 *  granice); liczony na KOŃCU, po wszystkich podbiciach limitu. */
function parametryDla(ep, body, sufit) {
  const b = { ...body };
  ...
  if (znane && znane.maxOut) {
    for (const k of ['max_tokens', 'max_completion_tokens']) if (typeof b[k] === 'number' && b[k] > znane.maxOut) b[k] = znane.maxOut;
  }
  /* Sufit osoby wygrywa z każdym podbiciem wyżej. Minimum 16 000 dla modeli
     myślących przebijało sufit członka na silniku przyznanym – przy domyślnym
     modelu Claude sufit 8192 nie działał wcale, a płacił właściciel. */
  if (Number.isFinite(sufit)) {
    for (const k of ['max_tokens', 'max_completion_tokens']) {
      if (typeof b[k] === 'number' && b[k] > sufit) b[k] = sufit;
    }
  }
  return b;
}
...
/** Odmowa „za duży limit odpowiedzi": pole „Maks. tokenów" jest wspólne dla
 *  wszystkich silników, a gpt-4o przyjmuje najwyżej 16 384 – podniesione dla
 *  Claude'a wyłączało zakładkę OpenAI przy każdej wiadomości (zespół IT,
 *  runda 4). Zwraca dopuszczalny limit albo 0. */
function limitWyjscia(tresc) {
  const m = String(tresc || '').match(/supports at most (\d+) completion tokens|>\s*(\d+), which is the maximum allowed number of output tokens/i);
 ...
 */
async function zapytajModel(ep, body, { signal, ponowienia = 2, sufit } = {}) {
  let b = parametryDla(ep, body, sufit);
  let poprawek = 0;
  let ponowien = 0;
  let przyciety = false;
  for (;;) {
    ...
    if (r.status === 400 && !przyciety) {
      ...
      const wyjscie = limitWyjscia(tresc);
      const limit = limitPoPrzepelnieniu(tresc) || wyjscie;
      if (limit && typeof b[klucz] === 'number' && limit < b[klucz]) {
        przyciety = true;
        b = { ...b, [klucz]: limit };
        // Sufit wyjścia modelu zapamiętany – następne pytanie idzie od razu dobre.
        if (wyjscie) {
          const kl = kluczModelu(ep, b.model);
          const znane = POPRAWKI.get(kl) || { usun: new Set(), naCompletion: false };
          znane.maxOut = wyjscie;
          POPRAWKI.set(kl, znane);
        }
        r.body?.cancel().catch(() => {});
        continue;
      }
...
async function llmComplete(messages, { endpoint = 'cloud', maxTokens = 1024, model: want, terminMs = 88_000, _ponowienie = false, _koniec } = {}) {
  ...
  const { modelDozwolony, tokenyDozwolone, granice } = require('./silniki.js');
  ...
  const sufit = granice(endpoint)?.maxTokens;
  if (!model) {
  ...
  /* Jeden termin na CAŁE wywołanie, z ponowieniem włącznie: 88 s, bo za
     Cloudflare żądanie bez odpowiedzi przez 100 s kończy się stroną 524
     zamiast czytelnego błędu. Dawniej każde wywołanie miało własne 90 s,
     a ponowienie po `length` liczyło od nowa – streszczenie modelem
     rozumującym kończyło się 524 po 100 s (zespół IT, runda 4). */
  const koniec = _koniec ?? Date.now() + terminMs;
  let r;
  try {
    r = await zapytajModel(ep, { model, messages, temperature: 0.7, max_tokens: maxTokens, stream: false },
      { signal: AbortSignal.timeout(Math.max(1, koniec - Date.now())), sufit });
  } catch (err) {
    if (err.name !== 'TimeoutError') throw err;
    throw new Error(`Model nie odpowiedział w wyznaczonym czasie (${Math.round(terminMs / 1000)} s). `
      + `Spróbuj krótszego tekstu albo szybszego modelu.  [${ep.label} · ${model}]`);
  }
  let d;
  try {
  ...
  // Ponowienie tylko wtedy, gdy zostało na nie dość czasu z tego samego terminu.
  if (why === 'length' && !_ponowienie && (msg.reasoning_content || msg.reasoning || think) && koniec - Date.now() > 30_000) {
    return llmComplete(messages, { endpoint, maxTokens: Math.min(16000, Math.max(4096, maxTokens * 4)), model: want, _ponowienie: true, _koniec: koniec });
  }
