...
const { katalogDla, WLASCICIEL_ID } = require('./kontekst.js');
...
/* `rozmowy` i `profil` to FUNKCJE. Profil wstrzyknięty jako napis przy starcie
   był zamrożony: zmiana profilu w Ustawieniach nie trafiała do zbioru
   treningowego aż do restartu serwera. */
let addEvent, rozmowy, convPath, profil;
function polacz(z) {
  ({ addEvent, rozmowy, convPath, profil } = z);
}
...
function buildTrainingDataset(format) {
  const sys = profil().trim();
  const out = [];
  for (const meta of rozmowy()) {
    let conv;
...
/* Trening jest wyłącznie dla właściciela (uruchamia procesy na serwerze),
   więc jego katalog to katalog właściciela — stały, niezależny od tego,
   kto akurat pyta o status. */
const TRAIN_DIR = path.join(katalogDla(WLASCICIEL_ID), 'train');
