...
const { zmyslyDozwolone } = require('./silniki.js');
...
async function wTle(res, rodzaj, opis, praca) {
  let z;
...
async function handleStudio(req, res, pathname) {
  ...
  if (pathname === '/api/studio/image' && req.method === 'POST') {
    ...
    return wTle(res, 'obraz', prompt, async () => {
      try {
        ...
        throw bladHttp(err.kod || 502, `Generowanie obrazu nie powiodło się: ${err.message}`);
      }
    });
  }
  ...
  if (pathname === '/api/studio/storyboard' && req.method === 'POST') {
    ...
    return wTle(res, 'storyboard', scene, async () => {
      try {
        ...
        throw bladHttp(err.kod || 502, `Storyboard nie powiódł się: ${err.message}`);
      }
    });
  }
  ...
  if (pathname === '/api/studio/edit' && req.method === 'POST') {
    ...
    return wTle(res, 'edycja', prompt, async () => {
      try {
        ...
        throw bladHttp(err.kod || 502, `Edycja obrazu nie powiodła się: ${err.message}`);
      }
    });
  }
  ...
  if (pathname === '/api/studio/upscale' && req.method === 'POST') {
    // Real-ESRGAN stoi na domowym GPU właściciela – ta sama zgoda co zmysły.
    if (!zmyslyDozwolone()) {
      return sendJson(res, 403, { error: 'Powiększanie działa na komputerze właściciela – dostęp daje przełącznik „lokalny GPU" w panelu Dostęp.' });
    }
    let data;
    try { data = await readJson(req); } catch { return sendJson(res, 400, { error: 'Nieprawidłowy JSON.' }); }
  ...
  if (pathname === '/api/studio/speech' && req.method === 'POST') {
    ...
    return wTle(res, 'dzwiek', text, async () => {
      try {
        ...
        throw bladHttp(err.kod || 502, `Generowanie dźwięku nie powiodło się: ${err.message}`);
      }
    });
  }
