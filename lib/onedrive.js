...
function utworz({ katalogDanych, clientId, clientSecret, redirectUri }) {
  ...
  function zapisz() {
    try {
      ...
      fs.writeFileSync(PLIK, JSON.stringify(stan), { mode: 0o600 });
    } catch (err) {
  ...
  function naWpis(el) {
    ...
    return {
      ...
      /* OBIEKTYWU tędy nie będzie i nie da się z tym nic zrobić: facet `photo`
         w Microsoft Graph podaje producenta, model korpusu, czas, przysłonę,
         ISO i ogniskową — ale NIE model obiektywu. W EXIF-ie z dysku jest
         (`LensModel`, tag 0xa434), z chmury nie przychodzi.

         Zostawiamy więc `null`, zamiast zgadywać po ogniskowej. Skutek jest
         widoczny i zamierzony: grupowanie archiwum po obiektywie pokaże dla
         materiału z OneDrive niskie pokrycie danych (`zDanymi` / `bezDanych`
         z Partii 31), czyli powie prawdę zamiast oddać pustą listę jak fakt. */
      obiektyw: null,
  ...
  return {
    skonfigurowany, polaczony, adresLogowania, polacz, indeksuj, rozlacz, graf,
