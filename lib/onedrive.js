...
function utworz({ katalogDanych, clientId, clientSecret, redirectUri }) {
  ...
  function zatrzymajNa(sekund) {
    ...
    przerwaDo = Math.max(przerwaDo, Date.now() + ile * 1000);
    dlawien++;
  }
  ...
  async function poczekajNaOkno(sygnal) {
    ...
    for (;;) {
      ...
      if (zostalo <= 0) { czekanoMs += Date.now() - start; return; }
      if (sygnal && sygnal.przerwane) { czekanoMs += Date.now() - start; return; }
  ...
  async function zadanieGraf(url, zbudujOpcje) {
    for (let proba = 0; ; proba++) {
      await poczekajNaOkno();
