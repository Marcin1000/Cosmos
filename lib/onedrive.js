...
function utworz({ katalogDanych, clientId, clientSecret, redirectUri }) {
  ...
  async function indeksuj(naPaczke, { folder = '', limit = 100000, sygnal } = {}) {
    ...
    const doOdwiedzenia = [`${korzen}?$top=200&$expand=thumbnails($select=small)`];
    let przejrzanych = 0;
    let dodanych = 0;
    ...
    while (doOdwiedzenia.length && dodanych < limit) {
      ...
      for (const el of strona.value || []) {
        ...
        if (el.folder) {
          ...
          doOdwiedzenia.push(`/me/drive/items/${el.id}/children?$top=200&$expand=thumbnails($select=small)`);
          continue;
        }
