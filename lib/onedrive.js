...
const { zapiszAtomowo, czytajJson } = require('./rdzen.js');
...
// Adresy z ENV tylko dla testów (atrapy Microsoftu); w produkcji nie ustawiaj.
const TOKEN = process.env.ONEDRIVE_TOKEN_URL || 'https://login.microsoftonline.com/consumers/oauth2/v2.0/token';
const GRAF = process.env.ONEDRIVE_GRAPH_URL || 'https://graph.microsoft.com/v1.0';
...
function utworz({ katalogDanych, clientId, clientSecret, redirectUri }) {
  ...
  // Brak pliku = niepołączone; uszkodzony — kopia obok i też niepołączone (czytajJson).
  let stan = czytajJson(PLIK, null);

  const skonfigurowany = () => Boolean(clientId && clientSecret && redirectUri);
  // Cofnięta zgoda (invalid_grant) = token do niczego; nie udajemy połączenia.
  const polaczony = () => Boolean(stan && stan.refresh_token && !stan.wymagaLogowania);
  const wymagaLogowania = () => Boolean(stan && stan.wymagaLogowania);
  ...
  async function wymienNaToken(pola) {
    ...
    if (!r.ok && d.error === 'invalid_grant' && pola.grant_type === 'refresh_token' && stan) {
      /* Zgoda cofnięta, hasło zmienione albo token wygasł po 90 dniach
         bezczynności. Każde kolejne żądanie skończyłoby się tym samym —
         zapamiętujemy, a interfejs prosi o ponowne połączenie. */
      stan.wymagaLogowania = true;
      zapisz();
      throw new Error('Microsoft cofnął dostęp do OneDrive — połącz konto ponownie w Ustawieniach.');
    }
    if (!r.ok) {
  ...
  /* Jedno odświeżanie naraz. Indeksowanie wysyła równolegle kilka żądań
     do Graph; po wygaśnięciu tokenu każde odświeżało go osobno (8 żądań =
     8 odświeżeń), a Microsoft przy rotacji refresh_token unieważnia
     poprzednie — część z nich kończyła się błędem. */
  let odswiezanie = null;
  async function token() {
    if (!polaczony()) throw new Error(wymagaLogowania()
      ? 'Microsoft cofnął dostęp do OneDrive — połącz konto ponownie w Ustawieniach.'
      : 'OneDrive niepołączony.');
    if (stan.access_token && Date.now() < stan.wygasa) return stan.access_token;
    odswiezanie ||= wymienNaToken({ refresh_token: stan.refresh_token, grant_type: 'refresh_token' })
  ...
  async function zadanieGraf(url, zbudujOpcje, pomiar = null) {
    ...
    for (let proba = 0; ; proba++) {
      ...
      /* 401 w połowie indeksowania: token wygasł wcześniej, niż mówił zegar
         (albo został unieważniony). Dawniej indeksowanie stawało na 800 z 8000
         plików, a status przez godzinę mówił „połączony". Odświeżamy raz. */
      if (r.status === 401 && !odswiezonyToken && stan && stan.refresh_token) {
        odswiezonyToken = true;
        stan.wygasa = 0;
        r.body?.cancel().catch(() => {});
        continue;
      }
      /* Przejściowy błąd serwera Microsoftu (500/502/504). Jeden taki przerywał
         całe indeksowanie (1000 z 8000). Dwie próby z krótką przerwą — tylko dla
         tego żądania, bez wspólnego hamulca, bo to nie jest dławienie. */
      if ([500, 502, 504].includes(r.status) && bladSerwera < 2) {
        r.body?.cancel().catch(() => {});
        await new Promise((ok) => setTimeout(ok, 2000 * 2 ** bladSerwera));
        bladSerwera++;
        continue;
      }
  ...
  async function graf(sciezka, pomiar = null) {
    ...
    if (!r.ok) {
      ...
      throw Object.assign(new Error(`Graph ${r.status}: ${tresc.slice(0, 200)}`), { status: r.status });
    }
    return r.json();
  }
   ...
   */
  /* WZNOWIENIE. Przejście po 2 TB trwa godzinami, a restart serwera
     (aktualizacja, awaria prądu) zaczynał je od korzenia — po raz kolejny
     te same dziesiątki tysięcy zapytań. `wznow` to stan kolejki zapisany
     przez `naPostep` po KAŻDEJ skończonej stronie: kolejka już bez niej,
     więc strona przerwana w połowie zostanie pobrana jeszcze raz (dopisanie
     tych samych wpisów jest bezpieczne — ten sam `id` nadpisuje). */
  async function indeksuj(naPaczke, { folder = '', limit = 100000, sygnal, wznow = null, naPostep = null } = {}) {
    const korzen = folder
      // Każdy segment osobno: `encodeURI` nie koduje „#" ani „?", a folder
      // „Sesja #3" docierał do Graph jako „Sesja ".
      ? `/me/drive/root:/${folder.replace(/^\/+/, '').split('/').map(encodeURIComponent).join('/')}:/children`
      : '/me/drive/root/children';
    // Kolejka z pliku tylko w poprawnym kształcie — `[123, null]` wywracało pętlę na zawsze.
    const zPliku = wznow && Array.isArray(wznow.doOdwiedzenia) && wznow.doOdwiedzenia.every((x) => typeof x === 'string');
    const doOdwiedzenia = zPliku ? [...wznow.doOdwiedzenia] : [`${korzen}?${ZAPYTANIE}`];
    let przejrzanych = (wznow && Number(wznow.przejrzanych)) || 0;
    let dodanych = (wznow && Number(wznow.dodanych)) || 0;

    while (doOdwiedzenia.length && dodanych < limit) {
      if (sygnal && sygnal.przerwane) break;
      /* Folder skasowany albo przeniesiony od zapisania kolejki (Marcin porządkuje
         2 TB): 404/410 znaczy „nigdy", nie „później". Dawniej całe wznowienie
         padało, kolejka zostawała, a każde „Indeksuj" wznawiało ten sam 404 —
         wyjście było tylko przez SSH (zespół IT, runda 4). Pomijamy i jedziemy dalej. */
      const adres = doOdwiedzenia.shift();
      let strona;
      try {
        strona = await graf(adres);
      } catch (err) {
        // Brak folderu, który wskazał człowiek, to błąd do pokazania — pomijamy tylko zniknięte po drodze.
        if (!BEZNADZIEJNE.has(err.status) || adres === `${korzen}?${ZAPYTANIE}`) throw err;
        if (naPostep) await naPostep({ doOdwiedzenia: [...doOdwiedzenia], przejrzanych, dodanych });
        continue;
      }
      const paczka = [];
    ...
    return { przejrzanych, dodanych, skonczone: !doOdwiedzenia.length };
  }
