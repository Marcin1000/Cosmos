...
/* Kształt ostatniej odpowiedzi — wyłącznie do diagnostyki.
 *
 * Prognoza wyszła kiedyś pusta przy adresie odpowiadającym w 10 ms i nie było
 * jak stwierdzić, CO właściwie przyszło. Czytnik rozpoznaje pola i przez to
 * jest odporny na trzy różne układy naraz — świetnie, tylko że wtedy nikt się
 * już nie dowie, który z nich NOAA naprawdę oddaje. A to jest potrzebne:
 * atrapa w testach zbudowana na złym założeniu uczy nieprawdy i utrwala ją
 * na lata. Jedno słowo w wydruku skryptu rozstrzyga to raz na zawsze. */
const OSTATNI_KSZTALT = new Map();

function opiszKsztalt(d) {
  if (Array.isArray(d)) {
    if (!d.length) return 'pusta tablica';
    const p = d[0];
    if (Array.isArray(p)) return `tablica tablic (${d.length})`;
    if (p && typeof p === 'object') {
      return `tablica obiektów (${d.length}; ${Object.keys(p).slice(0, 4).join(', ')})`;
    }
    return `tablica wartości (${d.length})`;
  }
  if (d && typeof d === 'object') return `obiekt (${Object.keys(d).slice(0, 4).join(', ')})`;
  return String(typeof d);
}

/** Jak wyglądała ostatnia odpowiedź spod tego adresu. `null`, gdy nie pytaliśmy. */
function ostatniKsztalt(adres) {
  return OSTATNI_KSZTALT.get(adres) || null;
}
...
async function pobierz(adres) {
  ...
  OSTATNI_KSZTALT.set(adres, opiszKsztalt(d));
  return d;
}
...
module.exports = {
  ...
  GRANICA_OWALU, ZASIEG_NAD_HORYZONTEM, ostatniKsztalt,
};
