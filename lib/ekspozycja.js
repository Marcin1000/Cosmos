...
function dobierz(ev, o = {}) {
  ...
  if (!Number.isFinite(ogniskowa) && temat && temat.ogniskowa) {
    const [tOd, tDo] = temat.ogniskowa;
    const kandydat = wybierzObiektyw(szkla, tOd).obiektyw;
    ogniskowa = kandydat
      ? Math.round(Math.min(Math.max(tOd, kandydat.od), Math.min(tDo, kandydat.do)))
      : Math.round((tOd + tDo) / 2);
    if (!Number.isFinite(ogniskowa) || ogniskowa <= 0) ogniskowa = Math.round((tOd + tDo) / 2);
  }
