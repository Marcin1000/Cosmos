...
function dobierz(ev, o = {}) {
  ...
  if (s.zgadywany) {
    powody.push(`„${s.nazwa}” nie ma w katalogu, więc liczone jak dla klasy `
      + `${KLASY_SPRZETU[s.klasa].nazwa} (${s.format}, ISO do ${s.isoMax}). `
      + 'Jeśli to nie ta półka, napisz to, a obliczenie się zmieni.');
  }
  ...
  if (ob) {
    powody.push(`Liczę dla ${ob.nazwa} przy ${ogniskowa} mm – najjaśniej f/${jasnoscTu}`
      ...
      + (ob.zgadywanaJasnosc ? ' Jasność obiektywu nie została podana, więc przyjęto f/4. Podaj ją, jeśli jest inna.' : ''));
  }
  ...
  if (docelowa) {
    ...
    if (Math.abs(zadana - przyslona) > 0.05) {
      powody.push(`Prosiłeś o f/${zadana}, ale ${ob ? ob.nazwa : s.nazwa} tego nie ma – `
        + `najbliżej jest f/${przyslona} i pod nią dobrane jest ISO.`
    ...
    if (tryb === 'wideo') {
      ...
      // O filtrze ND tylko wtedy, gdy światła jest ZA DUŻO. Nocą ta rada stała
      // obok „brakuje 9,6 działki światła” (agencja, runda 5).
      powody.push(brak < 0
        ? `Przysłona f/${przyslona} pod ${temat.nazwa}. Czasu ruszyć nie wolno `
          + '(reguła 180°), więc nadmiar światła zdejmij filtrem ND, a nie przymknięciem.'
        : `Przysłona f/${przyslona} pod ${temat.nazwa}. Czasu ruszyć nie wolno (reguła 180°).`);
  ...
  } else if (tryb === 'zdjecie' && Math.pow(2, ev) / Math.pow(najblizszy(przyslony, Math.max(8, przyslony[0])), 2) >= mianownik) {
    /* ZDJĘCIE PRZY DUŻYM ŚWIETLE: najpierw czas, potem przysłona, na końcu ND.
       Dawniej czas stał na najdłuższym z ręki, a cały nadmiar szedł
       w przymykanie: w południe 1/30 s, f/22 i „zostaje 1.3 działki
       prześwietlenia” (agencja, runda 5). f/8 to punkt wyjścia, f/16 sufit
       (dalej dyfrakcja zjada ostrość), resztę zdejmuje filtr ND. */
    przyslona = najblizszy(przyslony, Math.max(8, przyslony[0]));
    const najkrotszy = CZASY[CZASY.length - 1];
    const mozliwy = Math.pow(2, ev) / (przyslona * przyslona);
    if (mozliwy <= najkrotszy) {
      mianownik = najblizszy(CZASY, mozliwy);
    } else {
      mianownik = najkrotszy;
      const doSufitu = przyslony.filter((f) => f <= 16);
      przyslona = najblizszy(doSufitu.length ? doSufitu : przyslony, Math.min(16, Math.sqrt(Math.pow(2, ev) / najkrotszy)));
    }
    powody.push(`Dużo światła: czas 1/${mianownik} s przy f/${przyslona}, ISO bazowe.`);
  } else {
  ...
  if (roznica > 0.5) {
    powody.push(`Zostaje ${roznica.toFixed(1).replace('.', ',')} działki prześwietlenia – `
      + (tryb === 'wideo'
        ? 'to jest właśnie moment na filtr ND. Skracanie czasu zepsułoby regułę 180°.'
        : 'krótszego czasu i mocniejszego przymknięcia już nie ma sensu (dyfrakcja), więc załóż filtr ND.'));
  } else if (roznica < -0.5) {
    /* W wideo statyw nie doda światła (czasu nie wydłużysz, reguła 180°),
       a „światło zastane” znaczy dokładnie to, czego brakuje. Rada ma
       pasować do trybu (agencja, runda 5). Ułamek z przecinkiem. */
    const ile = Math.abs(roznica).toFixed(1).replace('.', ',');
    powody.push(`Brakuje ${ile} działki światła – `
      + (iso >= s.isoMax ? 'ISO jest już na maksimum, więc ' : '')
      + (tryb === 'wideo'
        ? 'potrzebne dodatkowe światło (lampa, panel LED) albo jaśniejszy obiektyw.'
        : 'potrzebny statyw i dłuższy czas, jaśniejszy obiektyw albo dodatkowe światło.'));
  }
  ...
  if (temat) {
    if (temat.zgadywany) {
      powody.push(`Tematu „${temat.nazwa}” nie ma w słowniku, więc liczone jak dla `
        + 'ujęcia w umiarkowanym ruchu. Jeśli obiekt jest szybszy albo zupełnie '
        + 'statyczny, napisz to, a czas się zmieni.');
    } else if (temat.uwagi) {
