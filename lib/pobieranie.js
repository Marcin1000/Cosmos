...
async function pobierzStrone(adres, {
  ...
  /* `timeout` gniazda to limit BEZCZYNNOŚCI — strona sącząca bajt co sekundę
     trwała 8 s przy limicie 2 s. Dokładamy limit CAŁKOWITY na całość,
     łącznie z przekierowaniami. */
  const koniecCzasu = Date.now() + czasMs;
  for (let krok = 0; krok <= maksPrzekierowan; krok++) {
    ...
    const odp = await new Promise((resolve, reject) => {
      ...
      const req = modul.request(url, {
        ...
        headers: {
          ...
          // Bez rozpakowywania po naszej stronie — prosimy o surowy tekst.
      ...
      req.on('error', reject);
      req.end();
      const zegar = setTimeout(() => req.destroy(new Error('Strona nie odpowiedziała na czas.')),
        Math.max(0, koniecCzasu - Date.now()));
      req.on('close', () => clearTimeout(zegar));
    });
    const zegarCiala = setTimeout(() => odp.destroy(new Error('Strona nie odpowiedziała na czas.')),
      Math.max(0, koniecCzasu - Date.now()));
    odp.on('close', () => clearTimeout(zegarCiala));
    ...
    // Serwer, który mimo `identity` przysłał skompresowane dane — nie udajemy,
    // że to tekst (wychodziły z tego śmieci w bazie wiedzy).
    const kodowanie = String(odp.headers['content-encoding'] || 'identity').toLowerCase();
    if (kodowanie !== 'identity') {
      odp.resume();
      return { status: 415, typ: '', tekst: '', adres: url.href };
    }
