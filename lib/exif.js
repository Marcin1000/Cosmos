...
/** Znajdź segment APP1 z XMP i oddaj go jako tekst.
 *
 *  XMP to DRUGI segment APP1 w tym samym pliku — obok EXIF-u, nie zamiast
 *  niego. Rozpoznaje się go po nagłówku `http://ns.adobe.com/xap/1.0/\0`,
 *  a w środku jest zwykły XML. To dlatego czytamy go osobno i inaczej:
 *  z EXIF-em łączy go tylko numer segmentu.
 *
 *  Dla materiału z drona to nie jest ozdoba. DJI wpisuje TUTAJ rzeczy,
 *  których w EXIF-ie nie ma w ogóle: wysokość nad punktem startu i kąty
 *  gimbala. „Z jakiej wysokości to leciało" i „kamera patrzyła w dół czy
 *  przed siebie" to pytania, na które inaczej nie da się odpowiedzieć.
 */
function znajdzXmp(buf) {
  if (buf.length < 4 || buf[0] !== 0xff || buf[1] !== 0xd8) return null;
  const NAGLOWEK = 'http://ns.adobe.com/xap/1.0/\0';
  let p = 2;
  while (p + 4 <= buf.length) {
    ...
    if (marker === 0xe1
        && buf.toString('latin1', p + 4, p + 4 + NAGLOWEK.length) === NAGLOWEK) {
      const od = p + 4 + NAGLOWEK.length;
      const doB = Math.min(buf.length, p + 2 + dlugosc);
      return buf.toString('utf8', od, doB);
    }
    p += 2 + dlugosc;
  }
  return null;
...
/** Jedno pole z XMP-a. DJI zapisuje je NA DWA SPOSOBY w tym samym pliku:
 *  jako atrybut (`drone-dji:RelativeAltitude="+42.30"`) i jako znacznik
 *  (`<drone-dji:RelativeAltitude>+42.30</drone-dji:RelativeAltitude>`).
 *  Który wypadnie, zależy od modelu i wersji oprogramowania, więc szukamy obu. */
function poleXmp(xmp, nazwa) {
  const atrybut = xmp.match(new RegExp(`${nazwa}\\s*=\\s*"([^"]*)"`, 'i'));
  if (atrybut) return atrybut[1];
  const znacznik = xmp.match(new RegExp(`<${nazwa}[^>]*>([^<]*)<`, 'i'));
  return znacznik ? znacznik[1] : null;
}

function liczbaXmp(xmp, nazwa) {
  const s = poleXmp(xmp, nazwa);
  if (s === null) return null;
  // DJI zapisuje ze znakiem wiodącym: „+42.30", „-90.00".
  const v = Number(String(s).trim().replace(/^\+/, ''));
  return Number.isFinite(v) ? v : null;
}

/** Dane lotu z XMP-a zdjęcia z drona. `null`, gdy to nie jest zdjęcie z DJI. */
function czytajXmpDrona(buf) {
  const xmp = znajdzXmp(buf);
  if (!xmp || !/drone-dji:/i.test(xmp)) return null;
  const out = {
    wysokoscWzgl: liczbaXmp(xmp, 'drone-dji:RelativeAltitude'),
    wysokoscBezwzgl: liczbaXmp(xmp, 'drone-dji:AbsoluteAltitude'),
    gimbalPochylenie: liczbaXmp(xmp, 'drone-dji:GimbalPitchDegree'),
    gimbalObrot: liczbaXmp(xmp, 'drone-dji:GimbalYawDegree'),
    dronObrot: liczbaXmp(xmp, 'drone-dji:FlightYawDegree'),
  };
