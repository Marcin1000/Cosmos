  ...
  async function proxySenses(req, res, targetPath, { json = false, search = '' } = {}) {
    ...
    try {
      ...
      return sendJson(res, 502, {
        error: czyWlasciciel()
          ? `Komputer domowy z usługą percepcji nie odpowiada (${SENSES_URL}): włącz go albo `
            + `uruchom na nim python senses/service.py. (${err.cause?.code || err.message})`
    ...
    try {
      ...
      return sendJson(res, 502, {
        error: `Usługa percepcji nie odpowiada pod ${SENSES_URL}. ` +
               `Włącz komputer domowy albo uruchom na nim python senses/service.py. (${err.cause?.code || err.message})`,
      });
    }
    const contentType = upstream.headers.get('content-type') || 'application/octet-stream';
    const buf = Buffer.from(await upstream.arrayBuffer());
