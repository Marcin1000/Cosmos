...
/* Reszta budżetu okna lokalnego: najpierw wypadają NAJSTARSZE tury rozmowy
   ...
   odpowiedzi schodzi do tego, co zostało. Jawnie — nagłówek X-Cosmos-Okno —
   zamiast cichego obcinania po stronie Ollamy, które zabierało początek
   promptu. Nie zmienia tablicy wejściowej. */
function przytnijDoOkna(wiadomosci, okno, maxTokens) {
  if (!okno) return { wiadomosci, przycietoTur: 0, limitZOkna: 0 };
  ...
  while (suma + naOdpowiedz > okno) {
    const i = zostaja.findIndex((m, idx) => m.role !== 'system' && idx < zostaja.length - 1);
    if (i < 0) break;
    suma -= szacujTokeny(zostaja[i].content);
    zostaja.splice(i, 1);
    przycietoTur++;
  }
  return { wiadomosci: zostaja, przycietoTur, limitZOkna: Math.max(512, Math.min(zadane, okno - suma - 32)) };
}
...
function utworz(z) {
  ...
  async function handleChat(req, res) {
    ...
    const { messages: zKontekstem, oknoLokalne } = await zlozKontekst(payload, ep);
    const { wiadomosci: messages, przycietoTur, limitZOkna } = przytnijDoOkna(zKontekstem, oknoLokalne, payload.max_tokens);
    const okno = przycietoTur ? `${oknoLokalne};${przycietoTur}` : '';
  ...
  return { handleChat, OCZEKUJACE, zlozKontekst };
}
