...
function utworz({ archiwum, onedrive, SENSES_URL, sendJson, readJson, addEvent,
  ...
  async function handleArchiwum(req, res, p) {
    ...
    if (p === '/api/archive/vision' && req.method === 'POST') {
      ...
      pula.dozwolone = Math.min(pula.dozwolone, ileNarazYolo);
      ...
      function poZadaniu() {
        ...
        } else if (pula.dozwolone < ileNarazYolo && Date.now() - pula.wzrostOd >= WZROST_MS) {
          pula.dozwolone++;
          pula.wzrostOd = Date.now();
        }
      }
  ...
  const pula = { dozwolone: Infinity, dlawienZnane: 0, wzrostOd: 0 };
