...
function utworz({ archiwum, onedrive, SENSES_URL, sendJson, readJson, addEvent,
  ...
  async function handleArchiwum(req, res, p) {
    ...
    if (p === '/api/archive/lenses' && req.method === 'POST') {
       ...
       *  w kolejce po jednym.
       *
       *  Sześć naraz to kompromis: Graph przy większej liczbie zaczyna
       *  odpowiadać 429, a wtedy nic nie zyskujemy — tylko dokładamy sobie
       *  odczekiwania. Wartość da się zmienić przez ONEDRIVE_RÓWNOLEGLE. */
      const ILE_NARAZ = Math.min(Math.max(Number(process.env.ONEDRIVE_RUWNOLEGLE) || 6, 1), 16);
      let nastepny = 0;
    ...
    if (p === '/api/archive/telemetry' && req.method === 'POST') {
      ...
      const ileNaraz = Math.min(Math.max(Number(process.env.ONEDRIVE_RUWNOLEGLE) || 6, 1), 16);
      let nastepnyKlip = 0;
