...
const { obrazBezpieczny, NAGLOWKI_CUDZEGO } = require('./rdzen.js');
const { kto } = require('./kontekst.js');
const { czytajSrt, naWpisArchiwum } = require('./srt.js');
const rawPodglad = require('./raw-podglad.js');

/* Ile bajtów początku pliku wystarczy, żeby zobaczyć, GDZIE leży podgląd.
   W TIFF-ie (CR2/NEF/DNG) to katalogi tagów tuż za nagłówkiem; w CR3 —
   nagłówek pudełka `moov`. Sto dwadzieścia osiem kilobajtów to ten sam
   kawałek, którym od dawna dociągamy EXIF, więc wiadomo, że schodzi szybko. */
const RAW_GLOWA_B = Number(process.env.ONEDRIVE_GLOWA_BYTES || 131072);

/* Jak wolno wracać do pełnej równoległości po dławieniu. Piętnaście sekund
   na jednego robotnika — bo przy dziewięciu żądaniach na sekundę „+1 po
   dwudziestu udanych" znaczyło powrót na pełny gaz w dwie sekundy, czyli
   prosto w kolejne 429. */
const WZROST_MS = Number(process.env.YOLO_WZROST_MS || 15000);

/* Jak wolno odtaje ZAPAMIĘTANA ŚCIANA — poziom, przy którym Microsoft
   powiedział dość. Dziesięć minut, bo kara za pomyłkę jest droga: zmierzone
   u Marcina `Retry-After` to około czterech minut postoju CAŁEJ puli. Przy
   takiej cenie ostrożność kosztuje kilka procent, a brawura kilkadziesiąt. */
const SUFIT_MS = Number(process.env.YOLO_SUFIT_MS || 600000);
...
function utworz({ archiwum, onedrive, SENSES_URL, sendJson, readJson, addEvent,
  ...
  /** Kto właśnie wgrywa paczkę do archiwum — jedna naraz na osobę. */
  const dodajeTeraz = new Set();
  ...
  async function handleArchiwum(req, res, p) {
    if (p === '/api/archive/add' && req.method === 'POST') {
      ...
      /* Porcjami, z oddechem dla pętli zdarzeń — paczka 5000 wpisów stawiała
         serwer na ~2 s dla wszystkich. Jedna paczka naraz na osobę: pętla
         takich żądań (a trasa jest dla każdego) nie zajmie całego procesora. */
      const osoba = kto().id;
      if (dodajeTeraz.has(osoba)) return sendJson(res, 429, { error: 'Poprzednia paczka jeszcze się indeksuje — spróbuj za chwilę.' });
      dodajeTeraz.add(osoba);
      let wynik;
      try { wynik = await archiwum.dodajPorcjami(d.wpisy); } finally { dodajeTeraz.delete(osoba); }
    ...
    if (p === '/api/archive/search' && req.method === 'GET') {
      ...
      /* STRONICOWANIE. Marcin: „chciałbym móc przejrzeć wszystkie np. zdjęcia
         z wyszukania, a nie mieć informację typu »pokazałem Ci 20, ale jest
         311«". Bez tego nie było jak — każde zapytanie oddawało ten sam
         początek listy, więc jedyną drogą do 311. pliku było zawężanie
         filtrów aż do skutku. Teraz przeglądarka dobiera kolejne porcje
         sama, tym samym zapytaniem, i doprowadza siatkę do końca.

         Parametr nazywa się `pomin`, a NIE `od` — `od` jest zajęte od dawna
         i znaczy „od tej daty". Pierwsza wersja użyła właśnie `od` i zestaw
         od razu to złapał: `od=24` wpadało do filtra dat jako „zdjęcia od
         roku 24", nie pasowało nic i druga porcja wracała pusta. Wyglądałoby
         to na koniec wyników i przycisk grzecznie by zniknął — po 24 plikach
         z 311. */
      const pomin = Math.max(0, Number(q.pomin) || 0);
      ...
      const strona = wyniki.slice(pomin, pomin + limit);
      return sendJson(res, 200, {
        znaleziono: wyniki.length,
        pomin,
        // Ile jeszcze zostało ZA tą stroną — przeglądarka nie musi tego liczyć.
        zostalo: Math.max(0, wyniki.length - (pomin + strona.length)),
        wyniki: strona,
        ...dodatki,
        ...podpowiedz,
      });
    ...
    if (p === '/api/archive/thumb' && req.method === 'GET') {
      ...
      try {
        ...
        if (!r.ok) return sendJson(res, 502, { error: 'To nie jest obraz.' });
        if (!obrazBezpieczny(typ)) return sendJson(res, 415, { error: 'Ten rodzaj obrazu nie jest przepuszczany.' });
      ...
      const jedenPlik = async (w) => {
        try {
          ...
          if (exif) {
            ...
            // Data z pliku jest pewna — zaznaczamy to, żeby kolejne listowanie
            // z Grapha nie podmieniło jej z powrotem na moment wgrania.
            if (exif.kiedy) scal.dataZrodlo = 'exif';
    ...
    if (p === '/api/archive/vision' && req.method === 'POST') {
      ...
      // Pierwsze uruchomienie: od pełnej puli. Kolejne: od tego, czego się
      // nauczyliśmy — ale nigdy powyżej sufitu z `.env`.
      pula.sufit = Math.min(pula.sufit, ileNarazYolo);
      pula.dozwolone = Math.min(pula.dozwolone, pula.sufit);
      ...
      /* PULA SAMA SCHODZI, GDY GRAPH PROSI O ZWOLNIENIE.
       *
       *  Stała liczba robotników nie działa, bo właściwa liczba nie jest
       *  stała. Zmierzone u Marcina na szesnastu: paczka z przewagą JPG-ów
       *  szła bez jednego 429, a paczka z przewagą RAW-ów zebrała cztery
       *  dławienia i minutę postoju — bo dla RAW-a Microsoft musi wygenerować
       *  podgląd i to jemu, nie nam, kończą się zasoby. Tej różnicy nie da
       *  się przewidzieć przed paczką: zależy od tego, na jaki fragment
       *  archiwum się trafi.
       *
       *  Gorsze było zachowanie po karze: przerwa jest wspólna, więc po jej
       *  końcu wszystkich szesnastu ruszało w tej samej milisekundzie i od
       *  razu dostawało kolejne 429. Stado biegnące na tę samą ścianę.
       *
       *  Więc: przy dławieniu połowimy dopuszczalną równoległość (nie mniej
       *  niż dwa), a po dwudziestu udanych żądaniach dokładamy po jednym,
       *  aż do pułapu. Start robotników rozsuwamy w czasie, żeby pierwsza
       *  fala też nie szła ławą. */
      /* STAN PULI PRZEŻYWA PACZKĘ — i to była cała różnica.
       *
       *  Pierwsza wersja trzymała `dozwolone` wewnątrz obsługi żądania, więc
       *  KAŻDA paczka zaczynała od nowa na pełnym gazie. Marcin opisał skutek
       *  dokładnie: „w ciągu 60 sekund potrafi zejść prawie 1000, ale później
       *  czeka dość sporo czasu, czyli leci to takimi falami". Panel to
       *  potwierdzał: cztery paczki `pula 16→16` po 14 sekund, a piąta
       *  `pula 16→8` z przestojem 264 s. Uczyliśmy się i natychmiast
       *  zapominali.
       *
       *  Drugi błąd był w tempie powrotu: „+1 po dwudziestu udanych" przy
       *  dziewięciu żądaniach na sekundę znaczyło powrót na pełną prędkość
       *  w dwie sekundy. Zejście o połowę i powrót w dwie sekundy to nie jest
       *  regulacja, tylko dokładnie ta fala.
       *
       *  Więc: schodzimy szybko (połowa przy każdym 429), wracamy powoli
       *  (najwyżej +1 na piętnaście sekund) i pamiętamy między paczkami. */
      let dolPuli = pula.dozwolone;
      const dlawien = () => (typeof onedrive.zdlawienia === 'function' ? onedrive.zdlawienia() : 0);
      let wLocieRazem = 0;
      if (pula.dlawienZnane < dlawienPrzed) pula.dlawienZnane = dlawienPrzed;
      async function bramka() {
        while (wLocieRazem >= pula.dozwolone) await new Promise((r) => setTimeout(r, 50));
      }
      function poZadaniu() {
        const teraz = dlawien();
        if (teraz > pula.dlawienZnane) {
          pula.dlawienZnane = teraz;
          /* ZAPAMIĘTUJEMY, GDZIE JEST ŚCIANA. Bez tego regulacja wracała
             w kółko do pułapu z `.env` — czyli tam, gdzie już raz dostała
             po łapach. Marcin: „około 4 minut przestoju pomiędzy falami",
             a panel: `pula 16→4`, `przestój na karze 248 s`. Kara jest
             podana przez Microsoft (`Retry-After`) i nie ma jak jej skrócić;
             jedyne wyjście to jej nie wywoływać. */
          pula.sufit = Math.max(2, Math.min(pula.sufit, pula.dozwolone - 1));
          pula.dozwolone = Math.max(2, Math.floor(pula.dozwolone / 2));
          pula.wzrostOd = Date.now();
          pula.sufitOd = Date.now();
          dolPuli = Math.min(dolPuli, pula.dozwolone);
        } else if (pula.dozwolone < pula.sufit && Date.now() - pula.wzrostOd >= WZROST_MS) {
          pula.dozwolone++;
          pula.wzrostOd = Date.now();
        } else if (pula.sufit < ileNarazYolo && Date.now() - pula.sufitOd >= SUFIT_MS) {
          /* Sufit też odtaje, tylko dużo wolniej — inaczej jedno dławienie
             z gorszej godziny zabetonowałoby pesymizm do końca przebiegu. */
          pula.sufit++;
          pula.sufitOd = Date.now();
        }
      }
      await Promise.all(Array.from({ length: ileNarazYolo }, async (_, nr) => {
        await new Promise((r) => setTimeout(r, nr * 40));
        for (;;) {
          const i = nastepnaGrupa++;
          if (i >= grupy.length) return;
          await bramka();
          wLocieRazem++;
          try { await jednaGrupa(grupy[i]); } finally { wLocieRazem--; }
          poZadaniu();
        }
      }));
      ...
      const st = (c) => {
        ...
        return { adres: s(c.adres), pobranie: s(c.pobranie), yolo: s(c.yolo),
          ...
          // Ile obrazków wzięliśmy z wnętrza pliku zamiast z renderowania Graph.
          zPliku: c.zPliku || 0 };
      };
      ...
      return sendJson(res, 200, {
        ...
        /* Do ilu robotników pula musiała zejść w tej paczce. Równe pułapowi
           znaczy „Graph nie protestował"; mniejsze — „tyle akurat przyjmuje".
           Wartość PRZECHODZI do następnej paczki, więc druga liczba mówi też,
           z czym startuje kolejna. */
        dolPuli: Math.min(dolPuli, pula.dozwolone),
        // Zapamiętana ściana: powyżej tego Graph już raz powiedział dość.
        sufitPuli: Number.isFinite(pula.sufit) ? pula.sufit : ileNarazYolo,
        // Średnie na ŻĄDANIE (nie na plik), w ms — widać, który etap kosztuje.
        czasy: st(czasy),
        czasyRaw: st(czasyRaw),
        /* ILU ROBOTNIKÓW NAPRAWDĘ PRACOWAŁO — suma zmierzonych etapów przez
           czas trwania paczki. To jest liczba, nie procent, i właśnie dlatego.

           Wcześniej stał tu procent liczony względem pułapu z `.env` i mylił
           przy każdym odczycie po zejściu puli: przy ścianie 3 i pułapie 16
           maksimum możliwe to 19%, więc pokazane „sprawność 12%" wyglądało na
           katastrofę, a było dwiema trzecimi tego, co się dało. Procent
           potrzebuje mianownika, którego nikt nie widzi; liczba robotników
           porównuje się wprost z sąsiednim „pula 16→3, ściana 3".

           Różnica między tą liczbą a dopuszczoną pulą to czas spędzony POZA
           mierzonymi etapami — i to ona wykryła, że zapis indeksu zamrażał
           pętlę zdarzeń na 22% czasu. */
        srednioNaraz: trwalo ? Math.round(
          ((czasy.adres + czasy.pobranie + czasy.yolo
            + czasyRaw.adres + czasyRaw.pobranie + czasyRaw.yolo) / trwalo) * 10) / 10 : 0,
  ...
  /* Ile robotników Graph AKTUALNIE przyjmuje. Poza obsługą żądania, bo ma
     przeżyć paczkę — patrz komentarz przy `poZadaniu`.
     `sufit` to zapamiętana ŚCIANA: poziom, przy którym Microsoft powiedział
     dość. Bez niego regulacja wraca w kółko tam, gdzie już raz dostała. */
  const pula = { dozwolone: Infinity, sufit: Infinity, dlawienZnane: 0, wzrostOd: 0, sufitOd: 0 };
  ...
  async function obiektyZMiniatury(idPliku, czasy = null, wLocie = null) {
    ...
    const t0 = Date.now();
    /* NAJPIERW PODGLĄD Z SAMEGO PLIKU, dopiero potem miniatura z Graph.
     *
     *  Dla RAW-a Graph musi wyrenderować podgląd u siebie i to jest ta
     *  różnica: JPG 309-1957 ms, RAW 7408-14101 ms, a 429 pojawiało się
     *  dokładnie w paczkach z przewagą RAW-ów. Aparat zapisał już gotowy
     *  podgląd wewnątrz pliku (to z niego korzysta ekranik z tyłu korpusu),
     *  a Graph obsługuje `Range` — więc bierzemy go stamtąd.
     *
     *  Gdy cokolwiek nie zagra, lecimy dalej starą drogą. To nie jest
     *  uprzejmość wobec dziwnych plików, tylko warunek, pod którym w ogóle
     *  warto było ten czytnik pisać: gorzej niż było być nie może. */
    const zPliku = await podgladZPliku(idPliku, czasy);
    if (zPliku) return rozpoznaj(zPliku, 'image/jpeg', czasy, wLocie);
    /* Kara za dławienie wypada WEWNĄTRZ tego wywołania, więc bez odjęcia
       jej doliczylibyśmy przestój do czasu etapu. Panel pokazywał wtedy
       „adres 16083 ms" dla zwykłego JPG-a — liczba, która kieruje szukanie
       winy na Microsoft, podczas gdy to nasza własna pauza. */
    const pomiar = { czekano: 0 };
    const el = await onedrive.graf(`/me/drive/items/${encodeURIComponent(idPliku)}`
      + '/thumbnails/0/large', pomiar);
    ...
    const buf = Buffer.from(await r.arrayBuffer());
    if (czasy) {
      czasy.adres += Math.max(0, (t1 - t0) - (pomiar.czekano || 0));
      czasy.pobranie += Date.now() - t1;
      czasy.bajty += buf.length;
    }
    return rozpoznaj(buf, typ, czasy, wLocie);
  }

  /** Podgląd JPEG wyjęty z samego pliku RAW — dwa żądania zakresowe zamiast
   *  jednego, ale bez czekania, aż Microsoft wyrenderuje podgląd u siebie.
   *  Zwraca `null` przy CZYMKOLWIEK nieoczekiwanym; wtedy wywołujący idzie
   *  starą drogą. */
  async function podgladZPliku(idPliku, czasy) {
    if (typeof onedrive.kawalekPliku !== 'function') return null;
    const t = Date.now();
    try {
      const glowa = await onedrive.kawalekPliku(idPliku, 0, RAW_GLOWA_B);
      if (!glowa || glowa.length < 16) return null;
      const plan = rawPodglad.zaplanuj(glowa);
      if (!plan) return null;
      /* Podgląd BYWA w tym, co już pobraliśmy — przy małych podglądach
         i przy CR3, gdzie `moov` siedzi zaraz za `ftyp`. Wtedy drugie
         żądanie byłoby zapłatą za to samo. */
      const wCalosci = plan.od + plan.ile <= glowa.length
        ? glowa.subarray(plan.od, plan.od + plan.ile)
        : await onedrive.kawalekPliku(idPliku, plan.od, plan.ile);
      const jpeg = rawPodglad.wyjmijJpeg(wCalosci, plan);
      if (!jpeg) return null;
      if (czasy) {
        czasy.pobranie += Date.now() - t;
        czasy.bajty += glowa.length + (wCalosci === glowa ? 0 : (wCalosci ? wCalosci.length : 0));
        czasy.zPliku = (czasy.zPliku || 0) + 1;
      }
      return jpeg;
    } catch {
      return null;               // każdy błąd = wracamy do miniatury z Graph
    }
  }

  /** Obrazek → etykiety YOLO. Wspólne dla obu dróg zdobycia obrazka. */
  async function rozpoznaj(buf, typ, czasy, wLocie) {
    const t2 = Date.now();
    /* SZCZYT JEDNOCZESNYCH ROZPOZNAŃ — ile naprawdę leci naraz do domu.
     *
     *  Pula ma kilkanaście robotników, ale to, ilu z nich stoi jednocześnie
     *  przy karcie graficznej, to zupełnie inna liczba: model jest jeden
     *  i chroni go zamek, więc kolejka ustawia się po tamtej stronie. Bez
     *  tego licznika nie da się odróżnić „karta wyrabia" od „karta jest
