  ...
  async function handleArchiwum(req, res, p) {
    ...
    if (p === '/api/archive/lenses' && req.method === 'POST') {
      ...
      const doZrobienia = archiwum.szukaj({ zrodlo: 'onedrive', typ: 'zdjecie' })
        .filter((w) => !w.obiektyw && !w.dron).slice(0, paczka);
      ...
      for (const w of doZrobienia) {
        try {
          ...
          /* Ten sam pobrany kawałek pliku niesie DWIE rzeczy: obiektyw z EXIF-u
             i dane lotu z XMP-a. Przy zdjęciu z drona obiektywu nie ma wcale,
             więc gdyby liczył się tylko on, materiał z Mavica wracałby do
             kolejki przy każdym przebiegu i nigdy nie zapisałby wysokości. */
          if (exif && (exif.obiektyw || exif.dron)) {
            archiwum.dodaj([{
              ...w,
              obiektyw: exif.obiektyw || w.obiektyw,
              dron: exif.dron || w.dron,
            }]);
            uzupelnione++;
          } else bezExifu++;
        } catch (err) {
      ...
      /* Kolejkę liczymy po tym, CO JUŻ SPRAWDZILIŚMY, a nie po samym braku
         obiektywu. Zdjęcie z drona nigdy obiektywu nie dostanie — bez tego
         rozróżnienia licznik „zostało" nie zszedłby nigdy do zera. */
      const zostalo = archiwum.szukaj({ zrodlo: 'onedrive', typ: 'zdjecie' })
        .filter((w) => !w.obiektyw && !w.dron).length;
    ...
    if (p === '/api/archive/telemetry' && req.method === 'POST') {
      ...
      return sendJson(res, 200, {
        sprawdzone: doZrobienia.length, odczytane, bezTelemetrii, zostalo, bledy,
      });
    }
