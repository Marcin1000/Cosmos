...
function utworz({ archiwum, onedrive, SENSES_URL, sendJson, readJson, addEvent,
  ...
  async function handleArchiwum(req, res, p) {
    ...
    if (p === '/api/archive/vision' && req.method === 'POST') {
       ...
       *
       *  Krok trzeci, na dwudziestu czterech: przez pięć paczek po dwanaście
       *  Graph nie poprosił o zwolnienie ANI RAZU (licznik `zdlawien`), więc
       *  zapas jest — a skoro czas idzie na czekanie, zapas warto zająć.
       *
      ...
      const ileNarazYolo = Math.min(Math.max(Number(process.env.YOLO_RUWNOLEGLE) || 24, 1), 32);
          ...
          for (const kandydat of grupa) {
            try {
              ...
              obiekty = await obiektyZMiniatury(String(kandydat.id).replace(/^onedrive:/, ''),
                kosztPliku(kandydat) < TANIE.length ? czasy : czasyRaw);
              pytano = true;
              break;
