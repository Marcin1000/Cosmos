...
function utworz({ archiwum, onedrive, SENSES_URL, sendJson, readJson, addEvent,
  ...
  async function handleArchiwum(req, res, p) {
    ...
    if (p === '/api/archive/vision' && req.method === 'POST') {
      ...
      let dozwolone = ileNarazYolo;
      let wLocieRazem = 0;
      let odSukcesu = 0;
      let dlawienZnane = dlawienPrzed;
      let dolPuli = ileNarazYolo;
      const dlawien = () => (typeof onedrive.zdlawienia === 'function' ? onedrive.zdlawienia() : 0);
      async function bramka() {
        while (wLocieRazem >= dozwolone) await new Promise((r) => setTimeout(r, 50));
      }
      function poZadaniu() {
        const teraz = dlawien();
        if (teraz > dlawienZnane) {
          dlawienZnane = teraz;
          dozwolone = Math.max(2, Math.floor(dozwolone / 2));
          dolPuli = Math.min(dolPuli, dozwolone);
          odSukcesu = 0;
        } else if (++odSukcesu >= 20 && dozwolone < ileNarazYolo) {
          dozwolone++;
          odSukcesu = 0;
        }
      }
