...
function utworz({ archiwum, onedrive, SENSES_URL, sendJson, readJson, addEvent,
  ...
  async function handleArchiwum(req, res, p) {
    ...
    if (p === '/api/archive/vision' && req.method === 'POST') {
       ...
       *
       *  Mniej równolegle niż przy EXIF-ie (tam cztery): każde żądanie kończy
       *  się rozpoznawaniem na JEDNEJ karcie graficznej w domu, więc nie ma
       *  sensu ustawiać przed nią długiej kolejki. Trzy wystarczą, żeby
       *  pobieranie następnej miniatury działo się w tle rozpoznawania
       *  poprzedniej — a to jest cały zysk. */
      const ileNarazYolo = Math.min(Math.max(Number(process.env.YOLO_RUWNOLEGLE) || 3, 1), 8);
      let nastepneZdjecie = 0;
      const czasy = { adres: 0, pobranie: 0, yolo: 0, bajty: 0, ile: 0 };
      const jedneZdjecie = async (w) => {
        try {
          const obiekty = await obiektyZMiniatury(String(w.id).replace(/^onedrive:/, ''), czasy);
          ...
          // Bez `obejrzane` — plik wróci do kolejki i spróbujemy jeszcze raz.
          if (bledy.length < 3) bledy.push(String(err.message).slice(0, 100));
        }
      };
      await Promise.all(Array.from({ length: ileNarazYolo }, async () => {
        for (;;) {
          const i = nastepneZdjecie++;
          if (i >= doZrobienia.length) return;
          await jedneZdjecie(doZrobienia[i]);
        }
      }));
      ...
      return sendJson(res, 200, {
        ...
        // Średnie na zdjęcie, w milisekundach — widać, który etap kosztuje.
