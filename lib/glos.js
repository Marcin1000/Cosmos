...
function utworz({ SENSES_URL, silniki, kto, sendJson, readBodyBuffer, readJson, env = process.env, STUDIO }) {
  ...
  async function handleStt(req, res) {
    ...
    if (z.online && silniki.zmyslyDozwolone(u)) proby.push(['zmysly', () => sttZmysly(audio, typ)]);
    if (wlasnyDla(u)) proby.push(['wlasny', () => sttOpenAiZgodny(cfg.sttBase, cfg.sttKey, cfg.sttModel, audio, typ, jezyk)]);
  ...
  async function handleTts(req, res) {
    ...
    for (const zrodlo of cfg.kolejnoscTts) {
      ...
      else if (zrodlo === 'zmysly' && z.online && silniki.zmyslyDozwolone(u)) zrob = () => ttsZmysly(tekst);
      if (!zrob) continue;
      try {
