...
/** Angielska wersja głowy strony produktowej (`/?lang=en`).
 *
 *  Roboty wyszukiwarek sprawdzają canonical w surowym HTML-u, a podgląd linku
 *  (LinkedIn, X, Messenger, Slack) w ogóle nie uruchamia skryptów – bez tej
 *  podmiany angielski adres mówił „jestem polską stroną /” po polsku. Teksty
 *  stoją raz, w atrybutach `data-en` w index.html: tu wartość angielska
 *  wchodzi na miejsce polskiej, a polska przechodzi do `data-pl`, skąd bierze
 *  ją strona.js przy powrocie na polski. Treść `<body>` zostaje – podmienia ją
 *  skrypt, a styl chowa stronę do tego czasu. */
function wersjaAngielska(html) {
  const cudzyslow = (t) => t.replace(/"/g, '&quot;');
  return String(html)
    .replace(/<html lang="pl"/, '<html lang="en"')
    .replace(/<title data-en="([^"]*)">([^<]*)<\/title>/, (_, en, pl) => `<title data-pl="${cudzyslow(pl)}">${en}</title>`)
    .replace(/<(meta|link)\b([^>]*?)\s(content|href)="([^"]*)"([^>]*?)\sdata-en="([^"]*)"([^>]*)>/g,
      (_, tag, przed, atr, pl, srodek, en, po) => `<${tag}${przed} ${atr}="${en}"${srodek} data-pl="${pl}"${po}>`);
}
...
function utworz({ PUBLIC_DIR }) {
  ...
  function serveStatic(req, res) {
    ...
    /* Bajt zerowy („/%00") wywracał odczyt pliku wyjątkiem z PEŁNĄ ścieżką
       instalacji – i to bez logowania. Zły adres, nie awaria. */
    ...
    let poAngielsku = false;
    if (urlPath === '/') {
      urlPath = '/strona/index.html';
      poAngielsku = /(?:^|&)lang=en(?:&|$)/.test(new URL(req.url, 'http://localhost').search.slice(1));
    }
    ...
    fs.readFile(filePath, (err, data) => {
      ...
      /* ETag i klucz pamięci skompresowanych liczone niżej z tego, co naprawdę
         wychodzi – obie wersje językowe mają własne. */
      if (poAngielsku) data = Buffer.from(wersjaAngielska(data.toString('utf8')), 'utf8');
...
module.exports = { utworz, MIME, KOMPRESUJ, skrotyInline, politykaAplikacji, wersjaAngielska };
