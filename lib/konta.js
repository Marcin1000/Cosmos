...
const { DATA_DIR, zapiszAtomowo } = require('./rdzen.js');
...
function czytaj(plik, domyslnie) {
...
/* 0600: te pliki zawierają skróty haseł i sesji. Nie ma powodu, żeby
   ktokolwiek poza procesem Cosmosa je czytał. */
function zapisz(plik, dane) {
  zapiszAtomowo(plik, JSON.stringify(dane, null, 2), { mode: 0o600 });
}

let uzytkownicy = czytaj(PLIK_UZYTKOWNICY, []);
let zaproszenia = czytaj(PLIK_ZAPROSZENIA, []);
let sesje = czytaj(PLIK_SESJE, []);

const zapiszUzytkownikow = () => zapisz(PLIK_UZYTKOWNICY, uzytkownicy);
...
function przeladuj() {
  uzytkownicy = czytaj(PLIK_UZYTKOWNICY, []);
...
function utworzSesje(uid, urzadzenie = '') {
  ...
  const u = znajdz(uid);
  if (u) { u.ostatnio = teraz; zapiszUzytkownikow(); }
  zapiszSesje();
  return token;
}
...
module.exports = {
  ...
  zaloguj, utworzSesje, uzytkownikSesji, usunSesje, usunSesjeUzytkownika, ileSesji,
  ...
  zablokowanyDo, zanotujPomylke, wyczyscPomylki, adresKlienta,
};
