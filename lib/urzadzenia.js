...
const { genId, readJson, saveJsonFile, sendJson } = require('./rdzen.js');
const { stan } = require('./kontekst.js');
const { llmComplete } = require('./model.js');
...
let addEvent, recentEvents, rutyny, routineView;
function polacz(z) {
  ({ addEvent, recentEvents, rutyny, routineView } = z);
}
...
/* Urządzenia KAŻDEJ OSOBY OSOBNO. Lista trafia do instrukcji modelu — przy
   wspólnej liście model gościa widziałby inteligentny dom właściciela
   i mógłby zaproponować, żeby go „włączyć". */
function D() {
  return stan('urzadzenia', (katalog) => {
    const plik = path.join(katalog, 'devices.json');
    let lista = [];
    try { lista = JSON.parse(fs.readFileSync(plik, 'utf8')); } catch { /* brak */ }
    return { plik, lista };
  });
}
const saveDevices = () => saveJsonFile(D().plik, D().lista);
...
async function handleDevices(req, res, pathname) {
  if (pathname === '/api/devices' && req.method === 'GET') {
    return sendJson(res, 200, { devices: D().lista });
  }
  if (pathname === '/api/devices' && req.method === 'POST') {
    ...
    const i = D().lista.findIndex((d) => d.id === dev.id);
    if (i >= 0) D().lista[i] = dev; else D().lista.push(dev);
    saveDevices();
    return sendJson(res, 200, { ok: true, id: dev.id });
  }
  if (pathname === '/api/devices' && req.method === 'DELETE') {
    const id = new URL(req.url, 'http://localhost').searchParams.get('id');
    D().lista = D().lista.filter((d) => d.id !== id);
    saveDevices();
  ...
  if (pathname === '/api/devices/run' && req.method === 'POST') {
    ...
    const dev = D().lista.find((d) => d.id === data.id)
      || D().lista.find((d) => d.name.toLowerCase() === want)
      || D().lista.find((d) => d.name.toLowerCase().includes(want));
...
module.exports = { BRIEFING, handleBriefing, handleDevices, polacz, urzadzenia: () => D().lista };
