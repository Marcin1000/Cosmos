...
/* Strefa czasu procesu = strefa właściciela. VPS chodzi zwykle w UTC, a Cosmos
   czyta czas bez strefy („kiedy=2026-06-21T19:30” w znaczniku PLAN, godziny
   w kontekście percepcji, granice doby dla Słońca) w strefie PROCESU. Na
   serwerze w UTC plan na 19:30 w Warszawie liczył się dla 21:30 i wychodziła
   „niebieska godzina” zamiast złotej (agencja, runda 5: rozmowa i backend).
   Node przyjmuje TZ ustawione w trakcie działania; jawne TZ z systemu wygrywa. */
if (!process.env.TZ) process.env.TZ = process.env.COSMOS_TZ || 'Europe/Warsaw';
...
function zapiszAtomowo(file, tresc, opcje = {}) {
  ...
  /* Nieudany zapis (pełny dysk) zostawiał `.tmp`. Przy 18-megabajtowym indeksie
     bazy wiedzy resztka zjadała ostatnie wolne miejsce i padały drobne zapisy
     INNYCH osób – profil, rozmowa, sesja (zespół IT, runda 4). Sprzątamy. */
  const sprzatnij = () => { try { fs.unlinkSync(tmp); } catch { /* nie ma czego */ } };
  try {
    ...
    try { fs.closeSync(fd); } catch { /* już zamknięty */ }
    sprzatnij();
    throw err;
  }
  fs.closeSync(fd);
  ...
  try { fs.renameSync(tmp, file); } catch (err) { sprzatnij(); throw err; }
}
...
// ---------------------------------------------------------------------------
/* OPIS BŁĘDU SIECI DLA CZŁOWIEKA. „fetch failed” i „The operation was aborted
   due to timeout” stały na ekranie w czacie, w „Pobierz listę”, w wyszukiwaniu
   i w grafikach (agencja, runda 5). Kod przyczyny siedzi w `err.cause.code`;
   z niego jedno konkretne zdanie. Kod techniczny zostaje w nawiasie na końcu,
   bo przydaje się przy zgłoszeniu. */
function opisBleduSieci(err, nazwa = 'usługa') {
  const kod = String(err?.cause?.code || err?.code || '');
  const czas = err?.name === 'TimeoutError' || /UND_ERR_CONNECT_TIMEOUT|ETIMEDOUT|UND_ERR_HEADERS_TIMEOUT/.test(kod);
  let zdanie;
  if (czas) zdanie = `${nazwa} nie odpowiada: minął czas oczekiwania. Spróbuj za chwilę.`;
  else if (/ENOTFOUND|EAI_AGAIN/.test(kod)) zdanie = `Serwer Cosmosa nie znalazł adresu: ${nazwa} (DNS). Sprawdź połączenie serwera z internetem albo adres w konfiguracji.`;
  else if (/ECONNREFUSED/.test(kod)) zdanie = `${nazwa} odrzuca połączenie: usługa nie działa pod tym adresem.`;
  else if (/EHOSTUNREACH|ENETUNREACH/.test(kod)) zdanie = `${nazwa} jest nieosiągalna z serwera Cosmosa (brak trasy w sieci).`;
  else if (/ECONNRESET|UND_ERR_SOCKET|EPIPE/.test(kod)) zdanie = `${nazwa} zerwała połączenie. Spróbuj jeszcze raz.`;
  else if (/CERT|SSL|TLS/i.test(kod)) zdanie = `${nazwa}: problem z certyfikatem HTTPS po drugiej stronie.`;
  else zdanie = `Serwer Cosmosa nie połączył się: ${nazwa}. Zwykle to chwilowy brak internetu na serwerze albo awaria usługi. Spróbuj za chwilę.`;
  return kod ? `${zdanie} (${kod})` : zdanie;
}

/* Strona błędu HTML zamiast JSON-a (503 Cloudflare'a albo nginx dostawcy) –
   jej treści nie pokazujemy, tylko mówimy, co znaczy. */
function toStronaHtml(tekst) {
  return /^\s*<(!doctype|html|head|body)/i.test(String(tekst || ''));
}

module.exports = {
  opisBleduSieci, toStronaHtml,
