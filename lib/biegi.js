...
function utworz({ zapiszOdpowiedz } = {}) {
  ...
  function zacznij({ id, rozmowaId, model, silnik, podmienionyZ, spozaListy, okno }) {
    sprzatnij();
    const b = {
      id,
      uzytkownik: kto(),
      ...
      okno: okno || '',               // „okno;przyciętych tur" – lokalny model z małym oknem
    ...
    /* Ten sam klucz już trwa (drugi POST z tym samym biegiem, który przeszedł
       mimo idempotencji w lib/czat.js): stary bieg przerywamy i oznaczamy jako
       zapisany. Nadpisany znikał z mapy, a pompa pisała dalej – restart go nie
       widział, a po 20 s lądował w rozmowie jako druga odpowiedź. */
    const stary = biegi.get(klucz(id));
    if (stary && !stary.koniec) { stary.zapisany = true; stary.przerwij?.(); zakoncz(stary, 'zastąpiony nowym żądaniem'); }
    b.puls = setInterval(() => {
      for (const w of b.widzowie) {
        if (!widzWazny(b, w)) continue;
  ...
  /* Widz, którego sesja zniknęła („Wyloguj wszędzie", zmiana hasła), dostawał
     odpowiedź do końca. Odpinamy go – bieg trwa dalej i zapisze się w rozmowie. */
  const waznoscWidza = new WeakMap();   // widz (res) → () => czy jego sesja jeszcze istnieje
  function widzWazny(b, w) {
    const wazny = waznoscWidza.get(w);
    if (!wazny || wazny()) return true;
    b.widzowie.delete(w);
    try { w.end(); } catch { /* już zamknięty */ }
    return false;
  }
  ...
  function potwierdz(id) {
    ...
    /* Odebrana i zapisana – zdarzenia nie są już nikomu potrzebne. Trzymane
       przez 15 min (TTL) dawały ~0,5 MB sterty na odpowiedź modelu
       rozumującego: przy dwunastu osobach setki megabajtów na odpowiedzi,
       które dawno leżą w rozmowach. Kto podepnie się teraz, dostanie 404
       i wczyta rozmowę – tam ta odpowiedź już jest. */
    if (b.koniec) { b.zdarzenia = []; b.tekst = ''; b.zwolniony = true; }
    return true;
  }
  ...
  function zapiszSierote(b, { mimoWidzow = false } = {}) {
    if (!zapiszOdpowiedz || !b.rozmowaId) return;
    {
      ...
      const bylNarzedzie = ZNACZNIKI.some((z) => new RegExp(`[\\[【]\\s*${z}\\s*[:：]`, 'i').test(surowe));
   ...
   *  `wazny()` – czy sesja widza jeszcze istnieje (sprawdzane przy każdym bloku). */
  function podepnij(id, od, res, { wazny } = {}) {
    const b = moj(biegi.get(klucz(id)));
    if (!b || b.zwolniony) return false;
    clearTimeout(b.timerSieroty);
    ...
    res.writeHead(200, {
      ...
      ...(b.okno ? { 'X-Cosmos-Okno': b.okno } : {}),
    });
    /* Nagłówki OD RAZU, nie z pierwszym kawałkiem. Model myślący po cichu nie
       wysyła nic przez długie sekundy, a przeglądarka bez ani jednego bajtu
       odpowiedzi traktuje zerwane połączenie jak niewysłane i sama ponawia
       POST – drugi płatny przebieg (zespół IT, runda 4). */
    res.flushHeaders?.();
    ...
    if (wazny) waznoscWidza.set(res, wazny);
    b.widzowie.add(res);
    res.on('close', () => {
  ...
  function lista() {
    ...
    return [...biegi.values()].filter((b) => moj(b)).map((b) => ({
      ...
      trwa: !b.koniec,
      zapisany: Boolean(b.zapisany),   // serwer zapisał już odpowiedź do rozmowy sam
      blad: b.blad,
