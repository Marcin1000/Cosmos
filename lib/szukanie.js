...
const { SEARCH_URL, sendJson, obrazBezpieczny, NAGLOWKI_CUDZEGO } = require('./rdzen.js');
...
/** Sama treść strony – bez menu, nagłówka, stopki, banera cookies.
 *  Model dostawał wcześniej „Portal Kategoria numer 0…" zamiast artykułu:
 *  z 2500 znaków na stronę połowę zjadała nawigacja, a właściwa liczba
 *  („7°C") w ogóle się nie mieściła. Najpierw szukamy <main> albo <article>;
 *  gdy są za krótkie (strony składane skryptem), bierzemy całe body bez
 *  elementów, które nigdy nie są treścią. */
function czytelnyTekst(html) {
  const bezSmieci = (h) => h
    .replace(/<!--[\s\S]*?-->/g, ' ')
    .replace(/<(script|style|noscript|svg|head|nav|header|footer|aside|form|button|iframe|template|select)\b[\s\S]*?<\/\1>/gi, ' ')
    .replace(/<(br|\/p|\/div|\/li|\/h[1-6]|\/tr|\/section|\/article)\s*\/?>/gi, '\n');
  const naTekst = (h) => stripTags(bezSmieci(h).replace(/\n\s*\n+/g, '\n'));
  for (const znacznik of ['main', 'article']) {
    const m = new RegExp(`<${znacznik}\\b[^>]*>([\\s\\S]*?)<\\/${znacznik}>`, 'i').exec(html);
    if (m) {
      const t = naTekst(m[1]);
...
async function fetchPageText(pageUrl, maxChars = 2500) {
  try {
    /* Adresy z wyników wyszukiwania też idą bezpieczną drogą (lib/pobieranie.js):
       wynik prowadzący do sieci prywatnej serwera jest nie do przyjęcia. */
    const r = await pobierzStrone(pageUrl, { czasMs: SZUKANIE_STRONA_MS, maxBajtow: 1_500_000 });
    if (r.status >= 400) return '';
    if (!/text\/html|text\/plain/i.test(r.typ)) return '';

    return czytelnyTekst(r.tekst.slice(0, 400000)).slice(0, maxChars);
  } catch {
    return '';                        // strona niedostępna albo zablokowana – zostają zajawki
  }
}
...
/* Doklej treść dwóch pierwszych stron. Zwraca, ilu trafieniom się udało.
 *
 * Musi obsłużyć OBIE drogi – SearXNG i DuckDuckGo. Wcześniej robiła to tylko
 * droga DuckDuckGo, a SearXNG zwracał wyniki od razu. Kto włączył SEARXNG_URL,
 * po cichu tracił to, dla czego `fetchPageText` w ogóle powstało: model
 * dostawał same tytuły i zajawki, nie znajdował w nich liczby, o którą pytano,
 * i prosił o kolejne wyszukiwanie aż do wyczerpania rund. Nagłówek tego pliku
 * obiecywał pobieranie treści bezwarunkowo – teraz jest to prawda.
 *
 * Równolegle, żeby nie sumować opóźnień: przy trzech rundach każda sekunda
 * mnoży się przez trzy.
 */
async function dolaczTresc(results) {
  const texts = await Promise.all(results.slice(0, 2).map((r) => fetchPageText(r.url)));
  texts.forEach((txt, i) => { if (txt) results[i].text = txt; });
  return texts.filter(Boolean).length;
}
...
async function handleSearch(req, res) {
  ...
  if (process.env.SEARXNG_URL) {
    try {
      ...
      if (results.length) {
        const withText = await dolaczTresc(results);
        addEvent('internet',
          `szukanie: „${q}" (${results.length} wyników, ${withText} z treścią – SearXNG)`);
...
/** Czy wolno pobrać ten adres: [kod, powód] albo null. Poza testami tylko https –
 *  proxy nie może służyć do zaglądania po http w sieć lokalną serwera. */
function zakazMiniatury(docelowy) {
  const lokalnaAtrapa = docelowy.origin === (() => {
    try { return new URL(DDG_START).origin; } catch { return null; }
  })();
  if (docelowy.protocol !== 'https:' && !lokalnaAtrapa) return [400, 'Tylko https.'];
  if (!HOSTY_MINIATUR.includes(docelowy.hostname)) return [403, 'Ten host nie jest na liście miniatur.'];
  return null;
}
const MINIATURA_MAX_PRZEKIEROWAN = 3;
...
async function handleImageProxy(req, res) {
  ...
  try {
    /* Przekierowania ręcznie, każdy krok przez te same bramki. `fetch` szedł za
       302 sam: adres z listy, który odsyła na localhost albo adres Tailscale,
       dawał każdemu zalogowanemu żądanie serwera w sieć domu właściciela
       (zespół IT, runda 4). */
    const termin = AbortSignal.timeout(SZUKANIE_STRONA_MS);
    let r;
    for (let krok = 0; ; krok++) {
      const zakaz = zakazMiniatury(docelowy);
      if (zakaz) return sendJson(res, zakaz[0], { error: zakaz[1] });
      r = await fetch(docelowy.href, {
        headers: { 'User-Agent': 'Cosmos/2.0', Referer: DDG_START },
        redirect: 'manual',
        signal: termin,
      });
      const dalej = r.status >= 300 && r.status < 400 && r.headers.get('location');
      if (!dalej) break;
      if (krok >= MINIATURA_MAX_PRZEKIEROWAN) return sendJson(res, 502, { error: 'Za dużo przekierowań.' });
      docelowy = new URL(dalej, docelowy);
    }
    const typ = r.headers.get('content-type') || '';
    if (!r.ok) return sendJson(res, 502, { error: 'To nie jest obraz.' });
    // SVG i reszta spoza listy – patrz obrazBezpieczny w lib/rdzen.js.
...
module.exports = {
  ...
  stripTags, czytelnyTekst, fetchPageText, decodeEntities, resolveDdgUrl,
