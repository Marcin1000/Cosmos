...
/* Słowa kluczowe to RDZENIE: pierwsze 5 liter. Polszczyzna odmienia, więc
   „Grilluję w soboty” i pytanie „pogoda na grilla w tę sobotę?” nie miały
   ani jednego wspólnego słowa i wspomnienie przepadało, gdy nie ma
   embeddingów (agencja, runda 5). „sobot”, „grill” łączą formy. */
const RDZEN = 5;
function keywords(text) {
  return new Set(
    text.toLowerCase().split(/[^a-ząćęłńóśźż0-9]+/i).filter((w) => w.length >= 4)
      .map((w) => w.slice(0, RDZEN))
  );
}
...
function utworz({ katalogDanych, sensesUrl, chmura, sendJson, readJson }) {
  ...
  async function handleMemory(req, res) {
    ...
    if (req.method === 'POST') {
      ...
      /* Krótko, jak w rozmowie: przy śpiącym komputerze domowym „Zapamiętaj"
         czekało minutę na wektor (zespół IT, runda 4). Bez wektora wpis i tak
         działa (słowa kluczowe), a brakujący dolicza się w tle przy pytaniu. */
      const vecs = await embedTexts([text], BUDZET_ZAPISU_MS, 'passage');
