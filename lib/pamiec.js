...
/** Wektor wpisu dla danego modelu albo null. */
function wektorDla(item, model) {
  if (!item || !model) return null;
  const w = item.wektory && item.wektory[model];
  if (Array.isArray(w) && w.length) return w;
  if ((item.embModel || null) === model && Array.isArray(item.embedding) && item.embedding.length) return item.embedding;
  return null;
}

/** Czy wpis ma jakikolwiek wektor. */
function maWektor(item) {
  if (item && item.wektory && Object.values(item.wektory).some((w) => Array.isArray(w) && w.length)) return true;
  return Boolean(item && Array.isArray(item.embedding) && item.embedding.length);
}

/** Dopisz wektor modelu do wpisu, zachowując te z innych modeli. */
function ustawWektor(item, model, wektor) {
  if (!model || !Array.isArray(wektor)) return;
  const w = { ...(item.wektory || {}) };
  // stary kształt → mapa, żeby nie zgubić wektora sprzed zmiany
  if (Array.isArray(item.embedding) && item.embedding.length && item.embModel && !w[item.embModel]) w[item.embModel] = item.embedding;
...
function utworz({ katalogDanych, sensesUrl, chmura, sendJson, readJson }) {
  ...
  let memories = czytajJson(MEMORY_FILE, []);
...
module.exports = { utworz, cosine, sameModel, keywordScore, wektorDla, ustawWektor, maWektor, MAKS_MODELI };
