...
const { zapiszAtomowo, authHeaders } = require('./rdzen.js');
...
function utworz({ katalogDanych, sensesUrl, chmura, sendJson, readJson }) {
  ...
  function saveMemories() {
    try {
      zapiszAtomowo(MEMORY_FILE, JSON.stringify(memories, null, 2));
    } catch (err) {
  ...
  async function searchMemory(query, limit = 4) {
    ...
    return memories
      .map((m) => {
        ...
        return { m, wektor, score: wektor ? cosine(qvec, m.embedding) : keywordScore(query, m.text) };
      })
...
module.exports = { utworz, cosine, sameModel, keywordScore };
