...
### 2-0. Domena w Cloudflare (raz, przed tunelem)
...
|---|---|---|
| SSL/TLS → Edge Certificates | **Always Use HTTPS**: włączone | bez HTTPS logowanie nie działa (`COSMOS_COOKIE_SECURE=1`) |
| Speed → Optimization | **Rocket Loader**: wyłączone | przepisuje skrypty, a aplikacja ma ścisłą politykę skryptów (CSP) |
| Caching → Configuration | **Browser Cache TTL**: Respect Existing Headers | Cosmos sam ustawia, co wolno trzymać w pamięci |
