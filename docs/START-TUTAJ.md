...
| „Pokaż zdjęcia z folderu Mazury 2026" nic nie znajduje | Naprawione — jest filtr `folder=`. Powiedz po prostu „z folderu Mazury 2026"; działa fragment nazwy, bez ogonków i wielkości liter („zdjecia mazury" trafia w „/Zdjęcia Mazury 2024 Dron/") |
| Cosmos twierdzi, że w archiwum są same zrzuty ekranu | Widział sześć najnowszych plików z 59 tysięcy i uczciwie z nich wnioskował. Teraz widzi kilkadziesiąt, bez adresów miniatur, i dostaje wprost napisane „to jest próbka". Przy pytaniach „ile" i „czego nie ma" ma używać zestawień (`grupuj=aparat`), które liczą CAŁOŚĆ |
| Zdjęcia z Canona wyglądają jak bez aparatu i z dzisiejszą datą | Microsoft Graph nie czyta metadanych z RAW-ów (CR2/CR3), więc zostaje data wgrania. Kliknij **Plener → Archiwum materiału → „Dociągnij dane z plików"** — Cosmos przeczyta EXIF prosto z pliku i uzupełni datę zrobienia, aparat, obiektyw, ISO i GPS. Idzie paczkami, można przerwać w każdej chwili |
| „Pokazałem Ci 20, ale jest 311" — chcę zobaczyć wszystkie | Pod siatką miniatur jest pasek „pokazane 24 z 311" i przycisk **„Pokaż kolejne"**. Klikaj do skutku — dojdziesz do ostatniego pliku. Model dalej widzi tylko próbkę (kontekst kosztuje) i **ma to napisane wprost**, więc nie powinien już przepraszać za limit ani proponować zawężania „żeby się zmieściło" |
| Wyniki nie idą od najnowszych — wyskakują zdjęcia sprzed lat | Sortowanie było dobre, kłamała data. Dla RAW-ów Graph nie podaje daty zrobienia zdjęcia, więc zostawała data **wgrania do OneDrive** — zdjęcie z 2022 wgrane wczoraj wyglądało na najnowsze. Teraz data z nazwy pliku (telefony i wysyłki z OneDrive datują nazwy) ma pierwszeństwo przed datą pliku, a **„Dociągnij dane z plików"** poprawia resztę z prawdziwego EXIF-u |
| Czy ponowne indeksowanie OneDrive skasuje rozpoznane treści? | **Nie** — i to była poważna usterka, naprawiona. Wcześniej listowanie podmieniało wpis w całości, więc jedno kliknięcie „indeksuj" po dograniu nowej sesji kasowało `obejrzane`, wykryte obiekty, obiektyw, dane lotu i poprawione daty. Teraz listowanie wnosi tylko to, co faktycznie wie, a owoce przebiegów po EXIF i po obrazie zostają. Indeksuj bez obaw |
| „Najnowsze, ale bez folderu Mazury 2026" i tak pokazuje Mazury | Naprawione — jest filtr odejmujący. Powiedz „oprócz folderu Mazury 2026"; można wymienić kilka naraz. Wcześniej filtrów było dwadzieścia i **ani jednego wykluczającego**, więc model pisał, że folder pominął, choć nie miał czym |
| Pytam o „Canon R6", a nic nie znajduje | Naprawione — nazwy sprzętu dopasowują się **po słowach**. EXIF zapisuje `Canon EOS R6m2`, a OneDrive pokazuje `EOS R6 Mark II`; jako jedna fraza „Canon R6" nie występuje nigdzie, więc dawniej trafiało w zero. Teraz „Canon R6" znajdzie oba zapisy i nadal nie pomyli R6 z R5 |
| W odpowiedzi widać `[ARCHIWUM: grupuj=rok]` albo pusty blok kodu | Naprawione. To polecenie dla modelu, nie treść dla Ciebie. Wyciekało, gdy model urwał je w pół (koniec budżetu tokenów lub „stop") albo opakował w blok kodu — po usunięciu znacznika zostawał sam pusty blok |
| Plan zdjęciowy liczy się dla domu, choć podałem inne miejsce | Naprawione. Geokoder dostawał całą frazę naraz („Cala d'Or, Hotel Barceló Ponent Beach") i nie znajdował nic, bo nie zna nazw hoteli. Teraz próbuje po kolei: pełna fraza, bez ostatniego członu, bez pierwszego. Możesz pisać miejsce tak, jak je nazywasz |
| Cosmos odpowiada dwa razy na jedno pytanie | Naprawione. Po **udanym** przeszukaniu archiwum drugie jest odcinane — model ma dane i ma odpowiedzieć. Po pustym wyniku drugie pytanie dalej wolno zadać, bo wtedy poprawka filtra ma sens |
| W odpowiedzi widzę `[SZUKAJ: …]` albo kilka takich linii | Naprawione. Gałąź wyszukiwania usuwała tylko pierwszy znacznik; gdy model napisał dziesięć, dziewięć zostawało na ekranie. Przy okazji model dowiaduje się teraz, ile z jego zapytań faktycznie poszło — wcześniej pisał tak, jakby miał wyniki wszystkich |
| Cosmos pisze o `swiatloPrzyblizone`, `grupuj=aparat`, Microsoft Graph | Naprawione, i to była **moja** wina, nie modelu: te zdania pochodziły z instrukcji, które sam mu dawałem. Jest teraz reguła oddzielająca to, co Cosmos wie o swojej mechanice, od tego, co mówi Tobie |
| Czy odłączenie OneDrive skasuje zaindeksowane zdjęcia? | **Nie** — i wcześniej kasowało, co było poważną usterką. Odłączenie tylko rozłącza konto; wyszukiwanie działa dalej, przestają się pokazywać miniatury. Do usuwania wpisów jest osobny przycisk **„Usuń zaindeksowane"** z ostrzeżeniem |
| Gdzie się podział opis „Podgląd działa, ale bez rozpoznawania…"? | Przeniósł się pod **ⓘ w prawym górnym rogu panelu kamery** — najedź myszą albo dotknij. To instrukcja do przeczytania raz, a wisząc nad podglądem zabierała jedną trzecią panelu na telefonie. Ikona pojawia się tylko wtedy, gdy jest co wyjaśnić |
| Tekst „Podgląd działa, ale…" wchodzi pod ramkę Nastaw | Naprawione. Obcięcie do dwóch linijek opierało się na `overflow`, a nie na jawnej wysokości — przy domyślnej wysokości wiersza rozstrzygał o nim font urządzenia i na niektórych telefonach cięcie wypadało w połowie wiersza. Teraz wysokość jest wielokrotnością wiersza z konstrukcji. Dotknij statusu, żeby rozwinąć go w całości |
| Po włączeniu trybu pełnoekranowego (⛶) treść ucieka poza prawą krawędź | Naprawione. Linijka nastaw `1/50 · f/5.6 · ISO 100` miała ustawione „nigdy się nie kurcz", a w foncie stałej szerokości zajmuje 190 px — w wąskim panelu wystawała o 20 px i wypychała poziomy pasek przewijania. Teraz się zawija |
| Na telefonie panel kamery jest wąską kolumną, nagłówek łamie się na dwie linijki | Naprawione. Szerokość panelu wyliczała się z wysokości — reguła dobra na desktopie, absurdalna na telefonie, bo kadr pionowy robił z ekranu 352 px panel na 206 px. Teraz na telefonie panel bierze **całą szerokość**, a ustępuje obraz (przycięty co najwyżej o połowę, sufit to 55% wysokości ekranu). Powiększony panel dalej pokazuje cały kadr |
| Telefon trzymany poziomo — sterowanie wypada pod krawędź ekranu | Naprawione. W poziomie ekran ma ~330 px wysokości, a same paski panelu ponad 200 — jedna kolumna nie miała szans. Przy niskim i szerokim ekranie panel przechodzi na **dwie kolumny**: obraz z lewej, nastawy z prawej. Na starym układzie ginęło 317–423 px sterowania |
| Na telefonie podgląd to wąski pasek, a po bokach czarne pasy | Naprawione. Scena trzyma teraz kształt kadru zawsze — przy kadrze pionowym panel się zwęża, zamiast ścinać obraz i dokładać czarne pasy. Zmierzone na oknie 390×844: obraz urósł ze 196 px do 615 px, pasy z 68% do zera |
| Komunikat „Podgląd działa, ale bez rozpoznawania…" zajmuje pół panelu | Naprawione — zwinięty do dwóch linijek, **dotknij, żeby rozwinąć** (tak jak pudełko nastaw). Na wąskim telefonie zajmował 144 px przy panelu wysokim na 441 px, czyli jedną trzecią, na instrukcję do przeczytania raz |
| Panel kamery: duży podgląd, a pod nim wąski pasek do przewijania | Naprawione. Na ekranie od 900 px powiększony panel ma teraz **dwie kolumny** — obraz z lewej, nastawy z prawej — więc nie konkurują o tę samą wysokość. Przy 1440×700 obraz urósł ze 189 px do 567 px. W rogu i na telefonie zostaje układ pionowy, ale gdy brakuje miejsca, najpierw kurczy się obraz, a nie sterowanie |
| „Mazury" wychodzą na Ukrainie | Naprawione — nazwy miejsc szukamy najpierw w Polsce (`GEOCODE_COUNTRY=pl`). Gdy w Polsce nic nie ma, zapytanie idzie globalnie, więc „Toskania" i „Lofoty" dalej działają |
...
| Pytam o zdjęcia „z Mazur" i Cosmos nic nie znajduje, choć mam taki folder | Naprawione. Filtr „miejsce" działa po GPS, a zdjęcia z lustrzanki go nie mają — teraz Cosmos sam zauważa, że ta sama nazwa jest w ścieżkach, i szuka po folderze. Możesz też od razu napisać „z folderu Mazury 2026" |
| Pytam o zdjęcia „przed 14" i dostaję same wieczorne | Naprawione. Dołożone filtry po godzinie zegarowej. Wcześniej Cosmos miał tylko „porę dnia", która liczy się z położenia Słońca — zdjęcie z 14:27 ma tam etykietę „wieczor", bo Słońce jest już na zachód od południa. To nadal jest widoczne w opisach miniatur; przy pytaniach o zegarek Cosmos używa teraz godzin |
| „Przerwane: kolejka nie maleje… Powód: Graph 429" | Naprawione. 429 znaczy „Microsoft każe zwolnić", nie „awaria". Cosmos odczekuje tyle, ile każe serwer Microsoftu, i próbuje dalej; w panelu widzisz wtedy „czekam N s". Gdyby zdarzało się to często, wpisz w `.env` na VPS-ie `ONEDRIVE_RUWNOLEGLE=2` i zrestartuj usługę |
| „Dociągnij dane z plików" idzie bardzo wolno przy dużym archiwum | Przyspieszone. Pliki idą po sześć naraz zamiast jeden po drugim, indeks schudł ośmiokrotnie (nie zapisujemy już adresów miniatur, które i tak wygasają), a zapis nie zatrzymuje serwera. Zostaw okno otwarte — paczki idą same aż do końca, „Przerwij" zatrzymuje między paczkami |
| „Rozpoznaj treść" pełznie — kilka zdjęć na sekundę przy dziesiątkach tysięcy w kolejce | Zmierzone było tak: adres 628 ms, **pobranie miniatury 6805 ms**, YOLO 220 ms — czyli prawie cały czas to czekanie, aż Microsoft wygeneruje podgląd z RAW-a (w pliku nie ma gotowego), przy karcie graficznej stojącej na 18%. Dla JPG-a w tej samej paczce było 451 ms. Stąd dwie zmiany: **dwanaście zdjęć naraz** zamiast trzech (czekanie zrównolegla się prawie za darmo) i **parowanie kadrów** — gdy ten sam kadr masz jako RAW i JPG, Cosmos pyta o JPG, a etykiety zapisuje obu plikom |
| Kadr mam jako RAW i JPG, ale w różnych folderach — czy Cosmos to połączy | Tak. Para rozpoznaje się po **nazwie pliku i sekundzie zdjęcia**, nie po ścieżce, więc działa też, gdy JPG-i leżą w osobnym folderze albo w podfolderze `jpg` obok RAW-ów, i niezależnie od tego, czy RAW to CR3, CR2, NEF czy format Samsunga. Zdjęcia z serii (kilka klatek w tej samej sekundzie) i te same nazwy z różnych wyjazdów zostają osobno. Gdy w panelu widzisz „N z pary RAW+JPG", to właśnie tyle plików dostało etykiety bez pytania o nie Microsoftu |
| Podgląd zdjęcia z lustrzanki (CR3, NEF…) ładuje się kilka sekund | Naprawione tym samym sposobem. Kafelek z RAW-em — w panelu archiwum i w siatce zdjęć w rozmowie — bierze podgląd z JPG-a tego samego kadru. Gdy JPG-a nie ma, idzie jak dotąd po RAW-ie |
| Rozpoznawanie nadal idzie wolniej, niż wynikałoby z czasów w panelu | Naprawione i to była najgrubsza rzecz w całej sprawie. Indeks archiwum (28 MB) przepisywał się co trzy sekundy, a `JSON.stringify` zamraża przy tym serwer na pół sekundy — czyli **co trzecią sekundę Cosmos stał przez dwie trzecie sekundy** i wszystkie trwające pobrania stały razem z nim. Teraz odstęp zapisu dobiera się do jego kosztu (najwyżej 5% czasu na zapisywanie), a indeks dopisuje się przed zamknięciem, więc restart niczego nie gubi. W panelu jest teraz „średnio N naraz" — ilu robotników naprawdę pracowało. Porównaj z sąsiednim „pula 16→N": gdy ta pierwsza liczba jest wyraźnie mniejsza, coś ich blokuje i warto mi to wkleić |
| Skąd wiem, ile jeszcze potrwa rozpoznawanie | `node scripts/pary-w-archiwum.js` na VPS-ie — czyta indeks, niczego nie zmienia, w kilkanaście sekund podaje ile żądań zostało i ile to godzin. Domyślnie liczy po 6100 ms na żądanie; podstaw swoją wartość z panelu („realnie N ms/żądanie") przez `MS_ZADANIE=3000 node scripts/pary-w-archiwum.js` |
| W panelu przy rozpoznawaniu pojawia się „Graph prosił o zwolnienie N×" | To Microsoft mówi, że dostaje za dużo żądań naraz. Cosmos i tak odczeka tyle, ile każe, i pójdzie dalej — nic nie przepada, ale kosztuje: przerwa jest wspólna dla całej puli, więc kilkanaście sekund kary zatrzymuje wszystkich robotników naraz. Obok zobaczysz „przestój na karze N s" — to jest ta cena. **Nie musisz nic robić:** Cosmos sam schodzi z równoległością przy dławieniu i sam wraca w górę, gdy Microsoft przestaje protestować. Widać to w panelu jako „pula 16→4". `YOLO_RUWNOLEGLE` w `.env` jest teraz tylko sufitem |
| Panel kamery nie mieści się na ekranie, a nagłówek albo przycisk migawki są ucięte | Naprawione, i były trzy powody naraz. Wysokość obrazu liczyła się przez odjęcie **stałej 220 px** na „nagłówek, źródło, status i przycisk" — a od czasu wpisania tej stałej doszło pudełko nastaw i status urósł do trzech wierszy. Teraz panel jest kolumną, w której paski biorą tyle, ile potrzebują, a obraz kurczy się, gdy brakuje miejsca. Do tego **wszystko pod obrazem da się przewinąć** (wcześniej panel miał `overflow: hidden` i treść po prostu znikała za krawędzią). I trzecia rzecz: obraz miał wpisane na sztywno proporcje 4:3, więc kamera 16:9 dawała czarne pasy na jedną czwartą wysokości — teraz proporcja bierze się z prawdziwego strumienia |
| Pod obrazem z kamery są trzy listy rozwijane i nie wiadomo, czego dotyczą | Naprawione. To są **nastawy kadru** — czas, przysłona i ISO policzone dla jasności tego, na co właśnie patrzysz (Plener liczy to samo, ale dla miejsca i godziny, bez kamery). Teraz pudełko jest **zwinięte do jednej linijki** z samymi nastawami, np. `1/50 · f/5.6 · ISO 100`; dotknięcie rozwija listy i uzasadnienie, a Cosmos zapamiętuje, jak je zostawiłeś. Zwinięte zabiera 42 px zamiast 266. Przy okazji: wcześniej **nie wypełniało się przy wyłączonych zmysłach** i pokazywało sam myślnik. Do policzenia ekspozycji karta graficzna nie jest potrzebna, więc teraz działa też przy wyłączonym komputerze domowym |
| Nad panelem kamery miga opis tego, co Cosmos widzi — a to samo jest pod obrazem | Naprawione. Dymek jest po to, żeby przy **zamkniętym** podglądzie dowiedzieć się, że Cosmos kogoś zobaczył. Przy otwartym powtarzał treść, która i tak stoi pod obrazem, i to na stałe zamiast na sześć sekund — teraz przy otwartym podglądzie milczy. Czujniki, urządzenia i rutyny nadal się pokazują, bo ich w podglądzie nie widać |
| Rozpoznawanie stoi po kilka minut między szybkimi odcinkami | Naprawione. Cosmos schodził z równoległością przy dławieniu, ale wracał potem do **pułapu z ustawień** — czyli tam, gdzie już raz dostał po łapach: rozpęd, kara, rozpęd, kara. Teraz zapamiętuje **ścianę** (poziom, przy którym Microsoft powiedział dość) i wraca tylko do niej. Widać to w panelu jako „pula 16→4, ściana 7". Samej kary nie da się skrócić — jej długość podaje Microsoft i bywa to kilka minut postoju całej puli — więc jedyne wyjście to jej nie wywoływać |
| Rozpoznawanie leci falami: przez minutę schodzi tysiąc, potem długo stoi | Naprawione. Cosmos schodził z równoległością przy dławieniu, ale **zapominał o tym przy każdej kolejnej paczce** i znowu ruszał pełnym gazem prosto w kolejną karę. Do tego wracał na pełną prędkość po dwudziestu udanych żądaniach, czyli po dwóch sekundach. Teraz schodzi szybko, wraca powoli (najwyżej o jednego robotnika na 15 s) i pamięta między paczkami. W panelu druga liczba przy „pula 16→N" mówi też, z czym startuje następna paczka |
| Tempo rozpoznawania raz rośnie, raz spada — czasem kilkakrotnie | Tak i to nie jest usterka: zależy, na jaki fragment archiwum akurat trafi paczka. Zmierzone `pobranie` miniatury to ~1-2 s dla JPG-a i 8-14 s dla RAW-a, bo dla RAW-a Microsoft musi wygenerować podgląd u siebie. Paczka z folderu ze zrzutami ekranu przelatuje, paczka z folderu CR3 pełznie. W panelu widzisz oba czasy osobno („JPG ×96 … RAW ×16 …"), więc od razu wiadomo, na co trafiłeś |
| Podgląd zdjęcia z lustrzanki wolniejszy niż z telefonu | Było tak, bo Microsoft musiał wyrenderować podgląd z RAW-a u siebie: zmierzone 764–1529 ms dla JPG-a wobec **7408–14101 ms dla RAW-a**, niezależnie od tego, ile plików szło naraz. Teraz Cosmos wyjmuje podgląd **z wnętrza samego pliku** — ten sam, który widzisz na ekraniku z tyłu korpusu — pobierając tylko dwa kawałki zamiast czekać na render. Działa dla CR3, CR2, NEF, DNG i TIF. Gdy pliku nie da się odczytać, wraca po miniaturę do Microsoftu, więc gorzej być nie może. W panelu widzisz to jako „RAW ×103 (z pliku 103)" |
| Odpowiedź urywa się w połowie zdania albo adresu | Naprawione. Cosmos rozpoznaje, że modelowi skończył się budżet długości, i sam prosi o dalszy ciąg, doklejając go bezszwowo. Domyślny limit odpowiedzi podniesiony z 2048 do 4096 tokenów (Ustawienia → „Maks. tokenów odpowiedzi" — możesz podnieść wyżej) |
| Klikam zdjęcie i od razu wyskakuje obca strona w nowej karcie | Naprawione. Kliknięcie otwiera podgląd w Cosmosie, w pełnej rozdzielczości, z przyciskiem „przejdź do źródła" i podpisem (serwis, licencja). Ctrl+klik nadal otwiera źródło w nowej karcie |
| „Szukam zdjęć: …" wisi pod znalezionymi już zdjęciami | Naprawione. Po znalezieniu komunikat zmienia się na „Zdjęcia: …", a Cosmos dopisuje, do których miejsc z planu należą |
| Prosiłem o zdjęcia do planu, a dostałem zdjęcia jednego miejsca | Naprawione. Cosmos pyta teraz o kilka miejsc naraz (do czterech w jednej rundzie) i po pokazaniu zdjęć sam prosi o kolejne przystanki |
| Odpowiedź z internetu bez klikalnych źródeł, za to z „【1†L1-L4】" | Naprawione. Każda odpowiedź oparta na wyszukiwaniu kończy się sekcją „Źródła:" z linkami. Gdy Cosmos odpowiada z własnej wiedzy, mówi to wprost zamiast wymyślać źródła |
