...
| W panelu przy rozpoznawaniu pojawia się „Graph prosił o zwolnienie N×" | To Microsoft mówi, że dostaje za dużo żądań naraz. Cosmos i tak odczeka tyle, ile każe, i pójdzie dalej — nic nie przepada, ale kosztuje: przerwa jest wspólna dla całej puli, więc kilkanaście sekund kary zatrzymuje wszystkich robotników naraz. Obok zobaczysz „przestój na karze N s" — to jest ta cena. Gdy pojawia się w co drugiej paczce, wpisz w `.env` na VPS-ie `YOLO_RUWNOLEGLE=8` i zrestartuj usługę; gdy przez kilka paczek jest zero, spróbuj w drugą stronę: `YOLO_RUWNOLEGLE=24` |
