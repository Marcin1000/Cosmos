...
| „Rozpoznaj treść" pełznie — kilka zdjęć na sekundę przy dziesiątkach tysięcy w kolejce | Przyspieszone dwa razy z dwóch stron. Zmierzone było tak: adres 628 ms, **pobranie miniatury 6805 ms**, YOLO 220 ms — czyli prawie cały czas to czekanie, aż Microsoft wygeneruje podgląd z pliku CR3 (w RAW-ie nie ma gotowego), przy karcie graficznej stojącej na 18%. Skoro fotografujesz w RAW+JPEG, obok każdego `.CR3` leży `.JPG` z tym samym kadrem: Cosmos pyta teraz o JPG, a etykiety zapisuje obu plikom. Kolejka topnieje o połowę, a każda pozycja jest tańsza. Do tego sześć zdjęć naraz zamiast trzech (`YOLO_RUWNOLEGLE` w `.env`, gdyby trzeba było zmienić) |
| Podgląd zdjęcia z lustrzanki (CR3) ładuje się kilka sekund | Naprawione tym samym sposobem. Kafelek z RAW-em — w panelu archiwum i w siatce zdjęć w rozmowie — bierze podgląd z leżącego obok JPG-a. Gdy JPG-a nie ma, idzie jak dotąd po RAW-ie |
