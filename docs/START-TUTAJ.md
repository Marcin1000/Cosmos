...
| `node scripts/zorza.js` pokazuje „Wpisów prognozy: 0" | Skrypt mówi teraz osobno o każdym źródle. `✗ SWPC 403/404` = NOAA nie przepuszcza żądań z tego serwera. `⚠ odpowiedziało, ale nie dało ani jednej wartości` = adres działa, ale zmienił się kształt danych albo produkt jest chwilowo pusty — skrypt podaje gotowy `curl` do sprawdzenia gołym okiem. Samo Kp bieżące wystarcza do werdyktu „czy TERAZ", ale nie „czy dziś w nocy", i skrypt to wypisuje |
