...
| Pod obrazem z kamery są trzy listy rozwijane i nie wiadomo, czego dotyczą | Naprawione. To są **nastawy na ten kadr** — czas, przysłona i ISO policzone dla jasności tego, na co właśnie patrzysz (Plener liczy to samo, ale dla miejsca i godziny, bez kamery). Pudełko ma teraz tytuł, a pola zawijają się do dwóch wierszy zamiast ucinać napisy. Przy okazji: wcześniej **nie wypełniało się przy wyłączonych zmysłach** i pokazywało sam myślnik. Do policzenia ekspozycji karta graficzna nie jest potrzebna, więc teraz działa też przy wyłączonym komputerze domowym |
