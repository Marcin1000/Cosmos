...
| W panelu przy rozpoznawaniu pojawia się „Graph prosił o zwolnienie N×" | To Microsoft mówi, że dostaje za dużo żądań naraz. Cosmos i tak odczeka tyle, ile każe, i pójdzie dalej — nic nie przepada. Gdyby liczba rosła przy każdej paczce, wpisz w `.env` na VPS-ie `YOLO_RUWNOLEGLE=6` i zrestartuj usługę. Gdy przez kilka paczek jest zero, możesz spróbować w drugą stronę: `YOLO_RUWNOLEGLE=24` |
